...
- 只有用户明确要求“开新分支”或任务包含需要隔离验证的高风险改动时，才切换到分支开发流程。
...
- 只有用户明确要求开分支，或任务本身属于高风险、长周期、需要隔离验证的实现时，才创建新分支或新 worktree。
...
- 只有当文档修改和实现代码、脚本、配置变更需要作为同一个可审阅单元提交，或用户明确要求隔离开发时，才使用 worktree。
