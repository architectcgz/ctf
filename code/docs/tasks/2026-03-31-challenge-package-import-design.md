...
### T1 重复 slug 导入改为拒绝，而不是覆盖更新

- 当前现状：
  - 普通题导入与 AWD 题导入都按 slug 查已有记录。
  - 如果 slug 已存在，当前实现会直接更新原题，而不是拒绝导入。
  - 前端导入确认页也没有明确提示“将覆盖已有题目”。
- 目标行为：
  - 上传题目包时，如果 `meta.slug` 已存在，应在预览或提交阶段明确拒绝导入。
  - 返回清晰错误，提示管理员该 slug 已被占用。
  - 如果管理员要更新已有题目，应走题目编辑/更新接口，而不是复用“导入新题目包”入口做隐式覆盖。
- 影响范围：
  - 普通题题目包导入链路
  - AWD 题目包导入链路
  - 导入预览/确认页的错误提示与交互文案
- 备注：
  - 这条规则和当前运行时容器命名约束是一致的：题目 slug 应当作为稳定唯一标识，而不是可被重复导入覆盖的软键。
