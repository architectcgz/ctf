...
- `internal/module/runtime` 统一负责实例运行时编排与资源治理，包括镜像信息查询、镜像删除、受管容器资源采集和实例运行时清理。
- `internal/module/runtimeinfra` 负责 Docker Engine、ACL 下发、运行时清理任务和资源指标采集等基础设施适配。
- `internal/module/system/dashboard_service.go` 只消费 runtime 模块暴露的受管容器指标，不再直接访问 Docker client。
...
- `internal/module/runtime` 为创建实例、销毁实例、延长实例、读取访问地址和查询实例列表新增带 `ctx` 的 service / repository 入口，实例相关 HTTP 路径不再在服务入口丢失请求上下文。
...
- `internal/module/runtimeinfra.Cleaner` 与 `internal/module/assessment.Cleaner` 现在都持有 service 级可取消 context，并在 `Stop(ctx)` 时取消正在执行的清理/重建任务，同时等待 cron 中已启动 job 退出。
...
- `internal/module/runtime/service.go` 为实例运行时清理、容器删除、网络删除和 ACL 清理补齐带 `ctx` 的入口，请求销毁、教师销毁、过期实例清理和孤儿容器清理不再统一退回 `context.Background()`。
