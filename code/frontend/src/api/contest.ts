...
export async function restartContestAWDServiceInstance(
  contestId: string,
  serviceId: string
): Promise<InstanceData> {
  const payload = await request<RawInstanceData>({
...
export async function requestContestAWDDefenseDirectory(
  ...
  return request<AWDDefenseDirectoryData>({
    method: 'GET',
    url: `/contests/${encodeURIComponent(contestId)}/awd/services/${encodeURIComponent(serviceId)}/defense/directories?path=${encodeURIComponent(path)}`,
  })
}
...
export async function requestContestAWDDefenseFile(
  ...
  return request<AWDDefenseFileData>({
    method: 'GET',
    url: `/contests/${encodeURIComponent(contestId)}/awd/services/${encodeURIComponent(serviceId)}/defense/files?path=${encodeURIComponent(path)}`,
  })
}
