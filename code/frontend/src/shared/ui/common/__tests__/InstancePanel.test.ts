...
describe('InstancePanel', () => {
  ...
  it('实例面板应接入共享表面与按钮原语，而不是继续依赖 Element Plus 卡片和按钮', () => {
    ...
    expect(instancePanelSource).toContain("from '@/entities/instance'")
  ...
  it('倒计时状态颜色应通过语义类承接，而不是从函数返回 Tailwind 任意主题类', () => {
    expect(instancePanelSource).toContain('getInstanceRemainingTone')
    expect(instancePanelSource).toContain('getInstanceStatusTone')
