...
const topNavStylesSource = readFileSync(
  resolve(process.cwd(), 'src/shared/ui/layout/topnav/topNavShell.css'),
  'utf8'
)
...
vi.mock('@/shared/model/layout', async () => {
  const actual = await vi.importActual<typeof import('@/shared/model/layout')>(
    '@/shared/model/layout'
  )
  return {
    ...actual,
...
vi.mock('@/shared/ui/layout/NotificationDrawer.vue', () => ({
  default: {
