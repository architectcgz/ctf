...
const derivedDownloadHint = computed(() => {
  ...
  return '后端当前未提供导出状态查询接口。请记录报告 ID，稍后可直接尝试下载。'
})
  ...
  <div class="space-y-6">
    <section class="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-6 shadow-sm">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div class="space-y-2">
          <p class="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-primary)]/80">Teacher</p>
          <div>
            ...
            <p class="mt-1 max-w-2xl text-sm leading-6 text-[var(--color-text-secondary)]">
              为指定班级生成训练报告。当前后端支持创建导出任务与按报告 ID 下载文件，不提供任务状态轮询。
            </p>
          </div>
        </div>

        <div class="rounded-xl border border-dashed border-[var(--color-border-default)] bg-[var(--color-bg-base)] px-4 py-3 text-sm text-[var(--color-text-secondary)]">
          当前账号：<span class="font-medium text-[var(--color-text-primary)]">{{ authStore.user?.username || '-' }}</span>
        </div>
      </div>
    </section>
    ...
    <section class="grid gap-6 xl:grid-cols-[1.25fr_0.9fr]">
      <div class="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-6 shadow-sm">
        ...
        <div class="space-y-5">
          ...
          <fieldset>
            <legend class="mb-2 text-sm font-medium text-[var(--color-text-primary)]">导出格式</legend>
          ...
          <div class="rounded-xl border border-[var(--color-border-default)] bg-[var(--color-bg-base)] px-4 py-3 text-sm text-[var(--color-text-secondary)]">
      ...
      <div class="space-y-6">
        <section class="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-6 shadow-sm">
          ...
          <div v-if="!latestExport" class="mt-4 rounded-xl border border-dashed border-[var(--color-border-default)] px-4 py-8 text-center text-sm text-[var(--color-text-secondary)]">
            还没有创建导出任务。
          </div>
          ...
          <div v-else class="mt-4 space-y-4">
            <div class="flex items-center justify-between gap-4">
              ...
              <span
                ...
                :class="latestExport.result.status === 'ready'
                  ? 'bg-emerald-500/12 text-emerald-600'
                  : 'bg-amber-500/12 text-amber-600'"
              >
                {{ latestExport.result.status === 'ready' ? '已就绪' : '生成中' }}
              </span>
            </div>
            ...
            <dl class="grid grid-cols-2 gap-3 text-sm">
              <div class="rounded-xl bg-[var(--color-bg-base)] px-4 py-3">
                ...
                <dd class="mt-1 font-medium text-[var(--color-text-primary)]">{{ latestExport.className }}</dd>
              </div>
              <div class="rounded-xl bg-[var(--color-bg-base)] px-4 py-3">
                <dt class="text-[var(--color-text-secondary)]">格式</dt>
                <dd class="mt-1 font-medium uppercase text-[var(--color-text-primary)]">{{ latestExport.format }}</dd>
              </div>
              <div class="rounded-xl bg-[var(--color-bg-base)] px-4 py-3">
                <dt class="text-[var(--color-text-secondary)]">创建时间</dt>
                <dd class="mt-1 font-medium text-[var(--color-text-primary)]">{{ formatDate(latestExport.createdAt) }}</dd>
              </div>
              <div class="rounded-xl bg-[var(--color-bg-base)] px-4 py-3">
                <dt class="text-[var(--color-text-secondary)]">过期时间</dt>
                <dd class="mt-1 font-medium text-[var(--color-text-primary)]">
                  {{ latestExport.result.expires_at ? formatDate(latestExport.result.expires_at) : '待生成完成后返回' }}
                </dd>
              </div>
            </dl>
            ...
            >
              {{ downloading ? '下载中...' : latestExport.result.status === 'ready' ? '下载报告' : '稍后尝试下载' }}
            </button>
          </div>
        </section>
        ...
        <section class="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-6 shadow-sm">
          ...
          <ol class="mt-4 space-y-3 text-sm leading-6 text-[var(--color-text-secondary)]">
            ...
            <li>2. 若状态为“已就绪”，可直接下载；若为“生成中”，请记录报告 ID。</li>
            <li>3. 当前后端没有状态查询接口，稍后可再次点击“稍后尝试下载”验证是否已生成完成。</li>
          </ol>
        </section>
      </div>
    </section>
