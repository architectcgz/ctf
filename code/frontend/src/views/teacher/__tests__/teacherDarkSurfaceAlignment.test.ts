...
describe('teacher dark surface alignment', () => {
  ...
  it('target pages should reuse shared journal and directory surface vocabulary instead of page-local skins', () => {
    ...
    expect(studentManagementSource).toContain('header-actions')
    expect(studentManagementSource).toContain('class="header-btn header-btn--ghost"')
    expect(studentManagementSource).not.toMatch(/class="teacher-(?:btn)/)
    expect(studentManagementSource).not.toContain('class="ui-btn ui-btn--secondary"')
    ...
    expect(instanceManagementSource).toContain('header-actions')
