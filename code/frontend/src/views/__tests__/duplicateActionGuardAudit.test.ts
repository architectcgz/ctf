...
import awdServiceTemplateEditorDialogSource from '@/components/platform/awd-service/AWDServiceTemplateEditorDialog.vue?raw'
...
describe('duplicate action guard audit', () => {
  it('表单 submit 与按钮 click 共用 handler 时应在本地 owner 上短路 in-flight 状态', () => {
    ...
    expect(awdServiceTemplateEditorDialogSource).toContain('if (props.saving) {')
