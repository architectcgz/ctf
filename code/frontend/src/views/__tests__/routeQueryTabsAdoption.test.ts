...
describe('route query tabs adoption', () => {
  it('admin 多 panel 页面应统一复用 useRouteQueryTabs，而不是继续在页面内手写状态机', () => {
    const tabSources = [
      ...
      adminChallengeDetailSource,
    ]

    for (const source of tabSources) {
      expect(source).toContain("import { useRouteQueryTabs } from '@/composables/useRouteQueryTabs'")
      expect(source).toContain('setTabButtonRef')
      expect(source).toContain(':ref="(element) => setTabButtonRef(tab.key, element as HTMLButtonElement | null)"')
    ...
    expect(userGovernanceSource).not.toContain('async function switchPanel(panelKey: UserPanelKey): Promise<void> {')

    expect(cheatDetectionSource).toContain('useRouteQueryTabs<CheatPanelKey>({')
    expect(cheatDetectionSource).not.toContain('const activePanel = computed<CheatPanelKey>(() => {')
