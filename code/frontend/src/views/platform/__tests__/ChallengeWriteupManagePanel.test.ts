...
describe('ChallengeWriteupManagePanel', () => {
  ...
  it('存在题解时应显示带边框的查看入口，并通过更多菜单进入编辑', async () => {
    ...
    expect(wrapper.findAll('.writeup-summary-grid .progress-card')).toHaveLength(2)
    ...
    expect(wrapper.findAll('.writeup-summary-grid .metric-panel-helper')).toHaveLength(2)
    expect(
    ...
    expect(wrapper.text()).toContain('学员题解1')
  ...
  it('题解投稿应显示作者与学号，并支持分页翻页', async () => {
    ...
    expect(wrapper.text()).toContain('学员题解8')
