...
interface AdminInstanceRow extends TeacherInstanceItem {
...
const instanceRows = computed<AdminInstanceRow[]>(() =>
...
const filteredRows = computed<AdminInstanceRow[]>(() => {
        ...
        >
          <template #cell-displayStudentCode="{ row }">
            <span class="admin-instance-manage-table__code">
              {{ (row as AdminInstanceRow).displayStudentCode }}
            </span>
          </template>
          ...
          <template #cell-displayStudentName="{ row }">
            <div class="admin-instance-manage-table__student-wrap">
              <span
                ...
                :title="(row as AdminInstanceRow).displayStudentName"
              >
                {{ (row as AdminInstanceRow).displayStudentName }}
              </span>
            </div>
          </template>
          ...
          <template #cell-challenge_title="{ row }">
            <span
              ...
              :title="(row as AdminInstanceRow).challenge_title"
            >
              {{ (row as AdminInstanceRow).challenge_title }}
            </span>
          </template>
          ...
          <template #cell-status="{ row }">
            <span
              ...
              :class="statusClass((row as AdminInstanceRow).status)"
            >
              {{ statusLabel((row as AdminInstanceRow).status) }}
            </span>
          </template>

          <template #cell-created_at="{ row }">
            <span class="admin-instance-manage-table__time-text">
              {{ formatDateTime((row as AdminInstanceRow).created_at) }}
            </span>
          </template>

          <template #cell-expires_at="{ row }">
            <span class="admin-instance-manage-table__time-text">
              {{ formatDateTime((row as AdminInstanceRow).expires_at) }}
            </span>
          </template>

          <template #cell-remaining_time="{ row }">
            <span class="admin-instance-manage-table__count-text">
              {{ formatRemainingTime((row as AdminInstanceRow).remaining_time) }}
            </span>
          </template>

          <template #cell-displayUrl="{ row }">
            <span class="admin-instance-manage-table__url-text" :title="(row as AdminInstanceRow).displayUrl">
              {{ (row as AdminInstanceRow).displayUrl }}
            </span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <button
              ...
              :disabled="destroyingId === (row as AdminInstanceRow).id"
              :data-instance-id="(row as AdminInstanceRow).id"
              @click="handleDestroy((row as AdminInstanceRow).id)"
            >
              {{ destroyingId === (row as AdminInstanceRow).id ? '销毁中...' : '销毁实例' }}
            </button>
          </template>
        </WorkspaceDataTable>
