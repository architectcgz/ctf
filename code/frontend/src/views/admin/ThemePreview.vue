  ...
  <div class="min-h-screen bg-[#f8fafc] text-slate-900 font-sans antialiased flex overflow-hidden">
    ...
    >
      <div class="h-16 flex items-center px-6 gap-3 border-b border-slate-800 shrink-0">
        ...
        <span v-if="!sidebarCollapsed || currentLayout === 'variant1'" class="font-bold text-white tracking-tight uppercase">CTF<span class="text-indigo-400">Ops</span></span>
      </div>
      
      <nav class="flex-1 p-4 space-y-2">
        <div v-for="i in 5" :key="i" :class="['flex items-center gap-3 px-4 py-3 rounded-xl transition-all cursor-pointer hover:bg-slate-800 hover:text-white', i === 1 ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-600/20' : '']">
          <component :is="[Layout, FileSearch, ShieldCheck, Users, Settings][i-1]" class="w-5 h-5" />
          <span v-if="!sidebarCollapsed || currentLayout === 'variant1'" class="text-sm font-bold">{{ ['概览中心', '题目管理', '竞赛编排', '用户治理', '系统设置'][i-1] }}</span>
        </div>
      </nav>
      ...
      <div class="p-4 border-t border-slate-800">
        <div class="flex items-center gap-3 px-4 py-3 text-slate-500">
          ...
          <span v-if="!sidebarCollapsed || currentLayout === 'variant1'" class="text-sm font-bold">退出系统</span>
        </div>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
      ...
      <header class="bg-indigo-900 text-white px-6 py-2 flex justify-between items-center z-[60] shrink-0">
        <div class="flex items-center gap-4">
          ...
          <div class="flex bg-black/20 p-1 rounded-lg">
            <button @click="currentLayout = 'variant1'" :class="['px-3 py-1 text-[10px] font-bold rounded-md transition-all', currentLayout === 'variant1' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-white/10']">方案1：导航融合</button>
            <button @click="currentLayout = 'variant2'" :class="['px-3 py-1 text-[10px] font-bold rounded-md transition-all', currentLayout === 'variant2' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-white/10']">方案2：分层指挥</button>
            <button @click="currentLayout = 'variant3'" :class="['px-3 py-1 text-[10px] font-bold rounded-md transition-all', currentLayout === 'variant3' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-white/10']">方案3：沉浸画布</button>
          </div>
        </div>
        <div class="text-[10px] font-mono opacity-50">PREVIEW MODE / {{ currentLayout.toUpperCase() }}</div>
      </header>
      ...
      <header class="bg-white border-b border-slate-200 shrink-0">
        <div class="px-8 h-16 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <button v-if="currentLayout === 'variant2'" @click="sidebarCollapsed = !sidebarCollapsed" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
        ...
        <div v-if="currentLayout === 'variant2'" class="px-8 flex gap-8">
          <button 
            v-for="t in ['manage', 'import', 'queue']" :key="t"
            @click="activeTab = t"
            :class="[
      ...
      <div class="flex-1 overflow-y-auto bg-[#f8fafc] p-8 custom-scrollbar">
        ...
        <!-- 如果是方案 1，Tab 放在内容顶部 -->
        <nav v-if="currentLayout === 'variant1'" class="flex gap-2 mb-8 bg-slate-200/50 p-1 rounded-2xl w-fit">
          <button v-for="t in ['manage', 'import', 'queue']" :key="t" @click="activeTab = t" :class="['px-6 py-2 rounded-xl text-xs font-bold transition-all', activeTab === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700']">
            {{ {manage: '题库管理', import: '上传题目包', queue: '导入队列'}[t] }}
          </button>
        </nav>
        ...
        <div class="max-w-[1400px] mx-auto">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <div v-for="s in stats" :key="s.label" class="bg-white border border-slate-200 rounded-[20px] p-6 shadow-sm hover:border-indigo-400 transition-all group relative overflow-hidden">
              <div class="flex flex-col h-full relative z-10">
                ...
                <div class="flex items-end justify-between">
                  <div class="flex flex-col">
                    ...
                    <p class="text-[10px] font-bold text-slate-400 mt-3 tracking-wide">核心指标说明</p>
                  </div>
                  <div :class="['text-right font-black text-[10px] px-2.5 py-1 rounded-lg border font-mono bg-white shadow-sm', s.colorClass]">{{ s.trend }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            <div class="relative w-full md:w-96">
              ...
              <input type="text" placeholder="检索资源..." class="w-full bg-white border border-slate-200 pl-11 pr-4 py-3 rounded-[18px] text-sm outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all" />
            </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
              <thead>
                <tr class="bg-slate-50/50 border-b border-slate-200 text-slate-400">
                  <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] w-[40%]">题目资源名称</th>
                  <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-center">分类</th>
                  <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-center">难度</th>
                  <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-center w-24">分值</th>
                  <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-right">操作</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                <tr v-for="c in challenges" :key="c.id" class="group hover:bg-slate-50/80 transition-colors">
                  <td class="px-8 py-5">
                  ...
                  <td class="px-6 py-5 text-center font-black text-slate-900 font-mono text-lg italic">{{ c.points }}</td>
                  <td class="px-8 py-5 text-right">
                    <div class="flex items-center justify-end gap-2">
                      <button class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"><Eye class="w-4 h-4" /></button>
                      <button class="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all"><MoreHorizontal class="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              </tbody>
