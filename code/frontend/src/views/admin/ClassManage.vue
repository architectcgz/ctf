...
interface AdminClassRow extends TeacherClassItem {
  code: string
...
const classRows = computed<AdminClassRow[]>(() =>
...
const filteredRows = computed<AdminClassRow[]>(() => {
...
function getRosterStatusLabel(row: AdminClassRow): string {
      ...
      <section class="workspace-directory-section admin-class-manage-directory">
        ...
        >
          <template #cell-code="{ row }">
            <span class="admin-class-manage-table__code">{{ (row as AdminClassRow).code }}</span>
          </template>

          <template #cell-name="{ row }">
            <div class="admin-class-manage-table__name-wrap">
              <span class="admin-class-manage-table__name-text" :title="(row as AdminClassRow).name">
                {{ (row as AdminClassRow).name }}
              </span>
            </div>
          </template>
          ...
          <template #cell-student_count="{ row }">
            <span class="admin-class-manage-table__count-text">
              {{ (row as AdminClassRow).student_count || 0 }}
            </span>
          </template>
          ...
          <template #cell-status="{ row }">
            <span
              ...
              :class="`admin-class-manage-table__status-pill--${(row as AdminClassRow).rosterStatus}`"
            >
              {{ getRosterStatusLabel(row as AdminClassRow) }}
            </span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <button
              ...
              @click="openClass((row as AdminClassRow).name)"
            >
              查看班级
            </button>
          </template>
