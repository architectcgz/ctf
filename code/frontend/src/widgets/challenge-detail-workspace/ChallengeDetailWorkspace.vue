<template>
  <section class="journal-shell journal-shell-user journal-hero workspace-shell min-h-full">
    ...
    <div v-else-if="challengeLoadState" class="challenge-detail-state">
      <AppEmpty
      ...
      >
        <template #action>
          <div class="challenge-detail-state__actions">
            <button type="button" class="ui-btn ui-btn--secondary" @click="goBackToChallengeList">
              返回题目列表
            </button>
            <button
    ...
    <ChallengeWorkspaceShell
      ...
      @update:selected-solution-id="setSelectedSolutionId"
      @change-submission-record-page="changeSubmissionRecordPage"
      @update:writeup-title="setWriteupTitle"
      @update:writeup-content="setWriteupContent"
...
const props = defineProps<{
  ...
  challengeLoadState: ChallengeLoadState | null
  ...
  needTarget: boolean
  openInstance: () => void | Promise<void>
  communitySolutions: CommunityChallengeSolutionData[]
  paginatedSubmissionRecords: SubmissionRecordItem[]
  recommendedSolutions: RecommendedChallengeSolutionData[]
  retryChallengeLoad: () => void | Promise<void>
  ...
  submissionSaving: EditableWriteupStatus | null
  ...
  submitFlagHandler: () => void | Promise<void>
  ...
  workspaceTabs: Array<{ id: WorkspaceTab; label: string }>
  writeupContent: string
  setWriteupContent: (value: string) => void
  writeupTitle: string
  setWriteupTitle: (value: string) => void
  saveWriteup: (status: EditableWriteupStatus) => void | Promise<void>
  destroyChallengeInstance: () => void | Promise<void>
  extendChallengeInstance: () => void | Promise<void>
