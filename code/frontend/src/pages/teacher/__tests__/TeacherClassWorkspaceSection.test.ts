...
import classWorkspaceSectionSource from '@/features/teaching/class-students-workspace/model/useClassWorkspaceSection.ts?raw'
...
describe('TeacherClassWorkspaceSection', () => {
  it('maps legacy detail entry routes back to the canonical class workspace with panel query state', () => {
    ...
    expect(classStudentsPageModelSource).toContain('useRouteQueryTransport')
    expect(classStudentsPageModelSource).toContain('useRouteNavigationTransport')
    expect(classStudentsPageModelSource).toContain('replace(canonicalWorkspaceTarget.value)')
    expect(classStudentsPageModelSource).not.toContain('router.replace(canonicalWorkspaceTarget.value)')
  })
})
