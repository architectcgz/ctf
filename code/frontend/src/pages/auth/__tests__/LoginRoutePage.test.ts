...
describe('LoginRoutePage', () => {
  ...
  function mountLoginRoutePage() {
    return mount(LoginRoutePage)
  }

  it('不应渲染 CAS 登录入口', async () => {
    const wrapper = mountLoginRoutePage()
  ...
  it('登录表单应切到共享控件原语而不是继续使用 Element Plus 表单', () => {
    expect(loginRoutePageSource).toContain('useLoginPage')
    ...
    expect(loginRoutePageSource).not.toContain('<ElButton')
  })
