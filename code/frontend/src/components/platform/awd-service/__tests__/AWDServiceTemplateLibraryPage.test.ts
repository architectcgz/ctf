...
describe('AWDServiceTemplateLibraryPage', () => {
  ...
  it('renders the import workspace as a standalone page mode', async () => {
    ...
    expect(wrapper.find('.awd-library-tabs').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('AWD 服务模板库')
  ...
  it('uses shared platform workspace surfaces instead of page-private premium panels', () => {
    ...
    expect(awdServiceTemplateLibraryPageSource).toContain('当前筛选条件下可管理的服务模板')
    expect(awdServiceTemplateLibraryPageSource).toContain('已开放给 AWD 题目编排使用的模板')
    expect(awdServiceTemplateLibraryPageSource).toContain('使用 HTTP 探测与 Web 服务模式的模板')
    expect(awdServiceTemplateLibraryPageSource).toContain('仍需完成 Checker 验证的模板')
    expect(awdServiceTemplateLibraryPageSource).toContain(
  ...
  it('keeps AWD template spacing on shared tokens instead of ad hoc page margins', () => {
    ...
    expect(awdServiceTemplateLibraryPageSource).not.toContain(
