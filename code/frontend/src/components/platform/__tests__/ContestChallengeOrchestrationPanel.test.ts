...
describe('ContestChallengeOrchestrationPanel', () => {
  ...
  it('应该提供题目池顺序快调按钮，并在 AWD 模式下按相邻题目交换顺序', async () => {
    contestApiMocks.listAdminContestChallenges.mockResolvedValue([
      buildChallenge({
        id: 'link-1',
        challenge_id: '101',
    ...
    expect(wrapper.get('#contest-challenge-move-up-link-2').text()).toContain('上移')
    expect(wrapper.get('#contest-challenge-move-down-link-1').text()).toContain('下移')

    await wrapper.get('#contest-challenge-move-up-link-2').trigger('click')
    await flushPromises()

    expect(contestApiMocks.updateContestAWDService).toHaveBeenNthCalledWith(1, 'contest-1', 'service-2', {
      order: 1,
    })
    expect(contestApiMocks.updateContestAWDService).toHaveBeenNthCalledWith(2, 'contest-1', 'service-1', {
      order: 2,
    })
  })

  it('应该提供显示和隐藏快调按钮，而不是只允许进入编辑弹窗', async () => {
    contestApiMocks.listAdminContestChallenges.mockResolvedValue([
      buildChallenge({
        id: 'link-1',
        challenge_id: '101',
    ...
    expect(wrapper.get('#contest-challenge-toggle-visibility-link-1').text()).toContain('隐藏')

    await wrapper.get('#contest-challenge-toggle-visibility-link-1').trigger('click')
    await flushPromises()
