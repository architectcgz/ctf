...
<template>
  ...
  >
    ...
    <template #footer="{ close }">
      ...
      >
        {{ props.creatingTeam ? '创建中...' : '确认创建' }}
      </button>
    </template>
  </CFocusedInputDialog>

  <CFocusedInputDialog
    :open="props.showJoinTeam"
    title="加入现有队伍"
  ...
  >
    ...
    <template #footer="{ close }">
      ...
      <button
        type="button"
        data-c-modal-action="primary"
        :disabled="props.joiningTeam"
        @click="emit('joinTeam')"
      >
        {{ props.joiningTeam ? '加入中...' : '确认加入' }}
      </button>
    </template>
  </CFocusedInputDialog>
</template>
