...
export function buildReviewWorkspaceSummaryItems(input: {
  sessionSummary?: {
...
export function buildReviewWorkspaceObservations(input: {
  ...
  const hasAWDSuccess = sessions.some(
    (session) => session.mode === 'awd' && session.result === 'success'
  )
...
function submissionResult(event: StudentEvidenceData['events'][number]): boolean | undefined {
  if (event.type !== 'challenge_submission' && event.type !== 'awd_attack_submission') {
    return undefined
  }
  const meta = event.meta ?? {}
  const isCorrect = meta.is_correct
  const isSuccess = meta.is_success
  if (typeof isCorrect === 'boolean') return isCorrect
  if (typeof isSuccess === 'boolean') return isSuccess
  return undefined
}
