import { listChallengeImports, previewChallengeImport } from '@/api/admin/authoring'
import type { AdminChallengeImportPreview } from '@/api/contracts'
import type { ShallowRef } from 'vue'

import { normalizeChallengeImportError } from './challengeImportErrorSupport'
...
interface UploadFlowToast {
  success: (message: string) => void
  warning: (message: string) => void
  error: (message: string) => void
}

interface UseChallengeImportUploadFlowOptions {
  toast: UploadFlowToast
  preview: ShallowRef<AdminChallengeImportPreview | null>
  uploading: ShallowRef<boolean>
  queueLoading: ShallowRef<boolean>
  selectedFileName: ShallowRef<string>
  queue: ShallowRef<AdminChallengeImportPreview[]>
  uploadResults: ShallowRef<ChallengePackageUploadResult[]>
}

const MAX_UPLOAD_RESULTS = 12

export function useChallengeImportUploadFlow({
  toast,
  preview,
  uploading,
  queueLoading,
  ...
  queue,
  uploadResults,
}: UseChallengeImportUploadFlowOptions) {
  ...
  async function selectPackage(file: File): Promise<AdminChallengeImportPreview | null> {
    ...
    try {
      ...
      const normalizedError = normalizeChallengeImportError(error)
  ...
  async function selectPackages(
    ...
    try {
      ...
      for (const item of settled) {
        ...
        const normalizedError = normalizeChallengeImportError(item.error)
        appendUploadResult('error', {
          fileName: item.file.name,
          message: normalizedError.message,
          code: normalizedError.code,
