      ...
      <div class="workspace-panel-header__meta flex flex-wrap gap-2">
        <template v-if="visibleWeakDimensions.length > 0">
          <template v-for="dim in visibleWeakDimensions" :key="dim">
            <ChallengeCategoryPill v-if="weakDimensionCategory(dim)" :category="weakDimensionCategory(dim)!" />
            <span
              v-else
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                  <span class="journal-soft-body-title text-sm font-semibold">{{ item.title }}</span>
