...
export function useAwdCheckResultPresentation({
  ...
  function getCheckStatusLabel(value: unknown): string {
    ...
    const labels: Record<string, string> = {
      ...
      tcp_connect_failed: 'TCP 连接失败',
      tcp_send_failed: 'TCP 发送失败',
      tcp_read_failed: 'TCP 读取失败',
      tcp_expectation_failed: 'TCP 响应不匹配',
      invalid_tcp_payload: 'TCP Payload 无效',
      invalid_tcp_expectation: 'TCP 期望无效',
