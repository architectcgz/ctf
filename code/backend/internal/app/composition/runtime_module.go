...
type RuntimeModule struct {
	...
	PracticeInstanceRepository interface{ FindByID(ctx context.Context, id int64) (*model.Instance, error); UpdateRuntime(ctx context.Context, instance *model.Instance) error; FinishActiveAWDServiceOperationForInstance(ctx context.Context, instanceID int64, status, errorMessage string, finishedAt time.Time) error; RefreshInstanceExpiry(ctx context.Context, instanceID int64, expiresAt time.Time) error; UpdateStatusAndReleasePort(ctx context.Context, id int64, status string) error; FindByUserAndChallenge(ctx context.Context, userID, challengeID int64) (*model.Instance, error); ListPendingInstances(ctx context.Context, limit int) ([]*model.Instance, error); TryTransitionStatus(ctx context.Context, id int64, fromStatus, toStatus string) (bool, error); CountInstancesByStatus(ctx context.Context, statuses []string) (int64, error) }
	PracticeRuntimeService     interface{ CleanupRuntime(ctx context.Context, instance *model.Instance) error; CreateTopology(ctx context.Context, req *practiceports.TopologyCreateRequest) (*practiceports.TopologyCreateResult, error); CreateContainer(ctx context.Context, imageName string, env map[string]string, reservedHostPort int) (containerID, networkID string, hostPort, servicePort int, err error) }
