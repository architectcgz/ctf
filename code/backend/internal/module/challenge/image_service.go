...
func NewImageService(repo *ImageRepository, dockerClient *client.Client, logger *zap.Logger) *ImageService {
	return &ImageService{
		repo:         repo,
...
func (s *ImageService) CreateImage(req *dto.CreateImageReq) (*dto.ImageResp, error) {
	...
	if err == nil && existing != nil {
		return nil, errcode.New(12101, "镜像已存在", 409)
	}
	...
	if s.dockerClient != nil {
		...
		if err != nil {
			return nil, errcode.New(12102, "Docker 镜像不存在或无法访问", 400).WithCause(err)
		}
	}
...
func (s *ImageService) GetImage(id int64) (*dto.ImageResp, error) {
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, errcode.New(12103, "镜像不存在", 404)
		}
		return nil, errcode.ErrInternal.WithCause(err)
	}
	return toImageResp(img), nil
...
func (s *ImageService) UpdateImage(id int64, req *dto.UpdateImageReq) error {
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return errcode.New(12103, "镜像不存在", 404)
		}
