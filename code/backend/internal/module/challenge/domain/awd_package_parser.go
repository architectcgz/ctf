...
func buildParsedAWDChallengePackage(
	...
	if !strings.EqualFold(strings.TrimSpace(manifest.Meta.Mode), "awd") {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("AWD 题目包必须声明 meta.mode = awd"))
	}
	...
	if strings.TrimSpace(manifest.Runtime.Type) != "container" {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("AWD 题目包必须提供 runtime.type=container"))
	}
	if runtimeImageRef == "" && dockerfilePath == "" {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("AWD 题目包必须提供 runtime.image.ref 或 docker/runtime/Dockerfile"))
	}
	...
	default:
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.service_type 仅支持 web_http、binary_tcp、multi_container"))
	}
	...
	default:
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.deployment_mode 仅支持 single_container、topology"))
	}

	checkerType := strings.TrimSpace(awd.Checker.Type)
	switch checkerType {
	case string(challengeentity.AWDCheckerTypeLegacyProbe), string(challengeentity.AWDCheckerTypeHTTPStandard), string(challengeentity.AWDCheckerTypeTCPStandard), string(challengeentity.AWDCheckerTypeScript):
	default:
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.checker.type 仅支持 legacy_probe、http_standard、tcp_standard、script_checker"))
	}

	flagMode := strings.TrimSpace(awd.FlagPolicy.Mode)
	if flagMode == "" {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.flag_policy.mode 不能为空"))
	}

	defenseEntryMode := strings.TrimSpace(awd.DefenseEntry.Mode)
	if defenseEntryMode == "" {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.defense_entry.mode 不能为空"))
	}
	...
	if len(accessConfig) == 0 {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.access_config 不能为空"))
	}
...
func resolveAWDPackageCheckerFiles(rootDir, checkerType string, checkerConfig map[string]any) (string, string, []ParsedAWDCheckerFile, error) {
	if checkerType != string(challengeentity.AWDCheckerTypeScript) {
		return "", "", nil, nil
	}
	rawEntry, ok := checkerConfig["entry"].(string)
	entry := strings.TrimSpace(rawEntry)
	if !ok || entry == "" {
		return "", "", nil, apperror.ErrInvalidParams.WithCause(errors.New("script_checker config.entry 不能为空"))
	}
	...
	if !entryIncluded {
		return "", "", nil, apperror.ErrInvalidParams.WithCause(errors.New("script_checker config.entry 必须包含在 config.files 中"))
	}
...
func validateAWDPackageDefenseWorkspace(rootDir string, runtimeConfig map[string]any) error {
	...
	if !ok {
		return apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.runtime_config.defense_workspace 不能为空"))
	}
	payload, ok := raw.(map[string]any)
	if !ok {
		return apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.runtime_config.defense_workspace 必须是对象"))
	}
	...
	if entryMode != "ssh" {
		return apperror.ErrInvalidParams.WithCause(errors.New("defense_workspace.entry_mode 当前仅支持 ssh"))
	}
	...
	if seedRoot == "docker" {
		return apperror.ErrInvalidParams.WithCause(errors.New("defense_workspace.seed_root 不能指向 docker 根目录"))
	}
...
func validateAWDPackageDefenseScope(rootDir string, runtimeConfig map[string]any) error {
	...
	if !ok {
		return apperror.ErrInvalidParams.WithCause(errors.New("extensions.awd.runtime_config.defense_scope 必须是对象"))
	}
	if _, exists := scope["editable_paths"]; exists {
		return apperror.ErrInvalidParams.WithCause(errors.New("defense_scope.editable_paths 已废弃，请改用 defense_workspace"))
	}
	...
	for _, required := range requiredProtected {
		if _, exists := protectedSet[required]; !exists {
			return apperror.ErrInvalidParams.WithCause(fmt.Errorf("defense_scope.protected_paths 必须包含 %s", required))
		}
	}
	return nil
}
...
func readAWDPackageRequiredString(payload map[string]any, key, field string) (string, error) {
	...
	if !ok || strings.TrimSpace(value) == "" {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 不能为空", field))
	}
...
func readAWDPackageDefenseWorkspaceDirList(rootDir string, raw any, field string) ([]string, error) {
	...
	for _, item := range items {
		...
		if _, exists := seen[clean]; exists {
			return nil, apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 存在重复路径: %s", field, clean))
		}
		seen[clean] = struct{}{}
...
func readAWDPackageDefenseWorkspaceDir(rootDir string, raw any, field string) (string, error) {
	...
	if isAWDDefenseWorkspaceProtectedPath(clean) {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 不能包含受保护路径: %s", field, clean))
	}
	abs, err := safePackageJoin(rootDir, clean)
	if err != nil {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 路径非法: %w", field, err))
	}
	info, err := os.Stat(abs)
	if err != nil {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 指向的目录不存在: %s", field, clean))
	}
	if !info.IsDir() {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 必须指向目录: %s", field, clean))
	}
	return clean, nil
}
...
func ensureAWDPackageDefenseWorkspaceInsideSeedRoot(seedRoot, value, field string) error {
	...
	return apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 必须位于 seed_root 内: %s", field, value))
}

func readAWDPackageRuntimeMounts(rootDir string, raw any) ([]awdPackageRuntimeMount, error) {
	rawList, ok := raw.([]any)
	if !ok || len(rawList) == 0 {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("defense_workspace.runtime_mounts 必须是非空数组"))
	}
	...
	for _, item := range rawList {
		...
		if !ok {
			return nil, apperror.ErrInvalidParams.WithCause(errors.New("defense_workspace.runtime_mounts 必须是对象数组"))
		}
		...
		if !strings.HasPrefix(target, "/") {
			return nil, apperror.ErrInvalidParams.WithCause(
...
func readAWDPackageDefenseScopePathList(raw any, field string) ([]string, error) {
	...
	for _, item := range items {
		...
		if _, exists := seen[clean]; exists {
			return nil, apperror.ErrInvalidParams.WithCause(fmt.Errorf("defense_scope.%s 存在重复路径: %s", field, clean))
		}
		seen[clean] = struct{}{}
		paths = append(paths, clean)
	}
...
func readAWDPackageStringList(raw any, field string) ([]string, error) {
	rawList, ok := raw.([]any)
	if !ok {
		return nil, apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 必须是非空字符串数组", field))
	}
	...
	if len(values) == 0 {
		return nil, apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 不能为空", field))
	}
	return values, nil
}
...
func normalizeAWDPackageRelativePath(value, field string) (string, error) {
	...
	if filepath.IsAbs(clean) || clean == "." || clean == ".." || strings.HasPrefix(clean, "../") {
		return "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 必须是题目包内相对路径", field))
	}
	return clean, nil
}
...
func validateAWDPackageExistingPath(rootDir, value, field string) error {
	...
	if _, err := os.Stat(abs); err != nil {
		return apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 指向的路径不存在: %s", field, value))
	}
	return nil
}
...
func readAWDPackageCheckerFileList(raw any) ([]string, error) {
	rawList, ok := raw.([]any)
	if !ok {
		return nil, apperror.ErrInvalidParams.WithCause(errors.New("script_checker config.files 必须是字符串数组"))
	}
	values := make([]string, 0, len(rawList))
	for _, item := range rawList {
		value, ok := item.(string)
		if !ok || strings.TrimSpace(value) == "" {
			return nil, apperror.ErrInvalidParams.WithCause(errors.New("script_checker config.files 必须是非空字符串数组"))
		}
...
func resolveAWDPackageCheckerFile(rootDir, value, fieldName string) (string, string, error) {
	...
	if filepath.IsAbs(clean) || clean == "." || clean == ".." || strings.HasPrefix(clean, ".."+string(filepath.Separator)) {
		return "", "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 必须是题目包内相对文件路径", fieldName))
	}
	abs, err := safePackageJoin(rootDir, clean)
	if err != nil {
		return "", "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 路径非法: %w", fieldName, err))
	}
	info, err := os.Stat(abs)
	if err != nil {
	...
	if info.IsDir() {
		return "", "", apperror.ErrInvalidParams.WithCause(fmt.Errorf("%s 不能是目录", fieldName))
	}
