...
func RenderAWDReviewArchiveZip(targetPath string, archive *assessmentqry.TeacherAWDReviewArchiveResp) (err error) {
	if archive == nil {
	...
	defer func() {
		if closeErr := file.Close(); err == nil && closeErr != nil {
			err = apperror.ErrInternal.WithCause(closeErr)
		}
	}()
...
func addAWDReviewEvidenceSampleSection(pdf *gofpdf.Fpdf, selected *assessmentqry.TeacherAWDSelectedRoundResp) {
	...
	for _, item := range attackRows[:min(len(attackRows), 3)] {
	...
	for _, item := range trafficRows[:min(len(trafficRows), 3)] {
...
func HottestRound(rounds []assessmentqry.TeacherAWDReviewRoundResp) *assessmentqry.TeacherAWDReviewRoundResp {
	...
	for _, round := range rounds[1:] {
		weight := round.AttackCount*100 + round.TrafficCount*10 + round.ServiceCount
...
func TopRiskyService(items []assessmentqry.TeacherAWDReviewServiceResp) *assessmentqry.TeacherAWDReviewServiceResp {
	...
	for _, item := range items[1:] {
		if serviceRiskScore(item) > serviceRiskScore(best) {
			best = item
		}
	}
	return &best
