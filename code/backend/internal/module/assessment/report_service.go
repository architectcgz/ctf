...
type classReportData struct {
	...
	DimensionAverages []ClassDimensionAverage
...
func (s *ReportService) buildClassReportData(ctx context.Context, className string) (*classReportData, error) {
	...
	return &classReportData{
		...
		DimensionAverages: fillMissingDimensionAverages(dimensionAverages),
...
func addAverageChart(pdf *gofpdf.Fpdf, title string, rows []ClassDimensionAverage) {
...
func addDimensionStatsTable(pdf *gofpdf.Fpdf, title string, rows []ReportDimensionStat) {
	pdf.SetFont("Helvetica", "B", 14)
	pdf.CellFormat(0, 8, sanitizePDFText(title), "", 1, "L", false, 0, "")
...
func addTopStudentsTable(pdf *gofpdf.Fpdf, title string, rows []ClassTopStudent) {
	pdf.SetFont("Helvetica", "B", 14)
	pdf.CellFormat(0, 8, sanitizePDFText(title), "", 1, "L", false, 0, "")
