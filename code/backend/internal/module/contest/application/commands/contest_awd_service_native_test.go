...
func TestContestAWDServiceServiceSnapshotRemainsFrozenAfterTemplateUpdate(t *testing.T) {
	...
	if err := challengeRepo.CreateAWDServiceTemplateWithContext(context.Background(), template); err != nil {
		t.Fatalf("create template: %v", err)
	}
	...
	if err := challengeRepo.UpdateAWDServiceTemplateWithContext(context.Background(), template); err != nil {
