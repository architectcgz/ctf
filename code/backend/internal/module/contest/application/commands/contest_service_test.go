...
func (s *stubContestRepository) Create(context.Context, *contestentity.Contest) error { return nil }
func (s *stubContestRepository) FindByID(context.Context, int64) (*contestentity.Contest, error) {
	return nil, contestdomain.ErrContestNotFound
}
func (s *stubContestRepository) Update(context.Context, *contestentity.Contest) error { return nil }
...
func TestContestServiceUpdateContestAllowsAWDStartOverride(t *testing.T) {
	...
	if resp == nil || resp.Status != contestentity.ContestStatusRunning {
...
func TestContestServiceUpdateContestRecordsManualStatusTransition(t *testing.T) {
	...
	if resp.Status != contestentity.ContestStatusRunning {
...
func TestContestServiceUpdateContestManualFreezeCreatesFrozenSnapshot(t *testing.T) {
	...
	resp, err := service.UpdateContest(context.Background(), 807, contestcmd.UpdateContestInput{
		Status: strPtr(contestentity.ContestStatusFrozen),
	})
	if err != nil {
		t.Fatalf("UpdateContest() error = %v", err)
	}
	if resp.Status != contestentity.ContestStatusFrozen {
		t.Fatalf("unexpected response: %+v", resp)
	}
...
func createContestForUpdateTest(t *testing.T, db *gorm.DB, contest *contestentity.Contest) {
	t.Helper()
