...
func TestAWDServicePreviewCheckerBroadcastsRealtimeProgressToRequester(t *testing.T) {
	...
	pending, err := outboxRepo.ListPendingRealtimeRelays(context.Background(), time.Now().UTC().Add(time.Second), 16)
	if err != nil {
		t.Fatalf("ListPendingRealtimeRelays() error = %v", err)
	}
	if len(pending) != len(expectedPhases) {
		t.Fatalf("expected %d pending realtime relays, got %+v", len(expectedPhases), pending)
	}
	for index, expectedPhase := range expectedPhases {
		relay := pending[index].Relay
		if relay.EventName != contestcontracts.EventAWDPreviewProgress {
			t.Fatalf("relay %d unexpected event name: %+v", index, relay)
		}
		if relay.RecipientUserID == nil || *relay.RecipientUserID != 9001 {
			t.Fatalf("relay %d unexpected recipient: %+v", index, relay)
		}
		payload, ok := relay.Payload.(contestcontracts.AWDPreviewProgressRelayPayload)
		if !ok {
			t.Fatalf("relay %d unexpected payload: %#v", index, relay.Payload)
		}
		if payload.PhaseKey != expectedPhase {
			t.Fatalf("relay %d unexpected phase_key: %+v", index, payload)
		}
		if payload.PreviewRequestID != "preview-progress-1" {
			t.Fatalf("relay %d unexpected preview_request_id: %+v", index, payload)
		}
		if payload.Status != "running" {
			t.Fatalf("relay %d unexpected status: %+v", index, payload)
		}
	}
	if payload, ok := pending[1].Relay.Payload.(contestcontracts.AWDPreviewProgressRelayPayload); !ok || payload.Attempt != 1 {
		t.Fatalf("attempt-1 relay missing attempt: %#v", pending[1].Relay.Payload)
	}
	if payload, ok := pending[3].Relay.Payload.(contestcontracts.AWDPreviewProgressRelayPayload); !ok || payload.Attempt != 3 {
		t.Fatalf("attempt-3 relay missing attempt: %#v", pending[3].Relay.Payload)
	}
}
