...
func TestAWDServiceCreateRoundAndListRounds(t *testing.T) {
	db := newAWDTestDB(t)
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), nil, "", config.ContestAWDConfig{}, zap.NewNop())
	now := time.Now()
...
func TestAWDServiceUpsertServiceCheckAppliesDefenseScore(t *testing.T) {
	...
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), redisClient, "", config.ContestAWDConfig{}, zap.NewNop())
	now := time.Now()
...
func TestAWDServiceRunCurrentRoundChecksRefreshesServices(t *testing.T) {
	...
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), redisClient, "", config.ContestAWDConfig{
...
func TestAWDServiceRunRoundChecksRefreshesSelectedRound(t *testing.T) {
	...
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), nil, "", config.ContestAWDConfig{
		CheckerTimeout:    time.Second,
		CheckerHealthPath: "/health",
	}, zap.NewNop())
...
func TestAWDServiceCreateAttackLogDeduplicatesScoringAndBuildsSummary(t *testing.T) {
	...
	scoreboardService := NewScoreboardService(NewRepository(db), redisClient, &config.ContestConfig{}, zap.NewNop())
...
func TestAWDServiceSubmitAttackUsesCurrentRoundFlagAndDeduplicatesByTeam(t *testing.T) {
	...
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), redisClient, "awd-secret", config.ContestAWDConfig{}, zap.NewNop())
	now := time.Now()
	...
	flag := buildAWDRoundFlag(4, 1, 412, 401, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackAcceptsPreviousRoundFlagWithinGrace(t *testing.T) {
	...
	service := NewAWDService(NewAWDRepository(db), NewRepository(db), redisClient, "awd-secret", config.ContestAWDConfig{
	...
	currentFlag := buildAWDRoundFlag(5, 2, 512, 501, "awd-secret", "awd")
	...
	previousFlag := buildAWDRoundFlag(5, 1, 512, 501, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackAllowsFrozenContest(t *testing.T) {
	...
	flag := buildAWDRoundFlag(6, 1, 612, 601, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackIgnoresStaleCurrentRoundPointer(t *testing.T) {
	...
	currentFlag := buildAWDRoundFlag(7, 2, 712, 701, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackUsesTimeDerivedCurrentRoundWhenRoundStatusLags(t *testing.T) {
	...
	currentFlag := buildAWDRoundFlag(8, 2, 812, 801, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackRejectsPreviousFlagAfterMaterializingMissingCurrentRound(t *testing.T) {
	...
	previousFlag := buildAWDRoundFlag(9, 1, 912, 901, "awd-secret", "awd")
...
func TestAWDServiceSubmitAttackMaterializesMissingCurrentRound(t *testing.T) {
	...
	currentFlag := buildAWDRoundFlag(10, 2, 1012, 1001, "awd-secret", "awd")
...
func assertContestRedisScore(t *testing.T, redisClient *redis.Client, contestID, teamID int64, expected float64) {
	...
	score, err := redisClient.ZScore(context.Background(), rediskeys.RankContestTeamKey(contestID), teamIDToMember(teamID)).Result()
	if err != nil {
...
func assertContestRedisScoreMissing(t *testing.T, redisClient *redis.Client, contestID, teamID int64) {
	...
	_, err := redisClient.ZScore(context.Background(), rediskeys.RankContestTeamKey(contestID), teamIDToMember(teamID)).Result()
	if err == nil {
