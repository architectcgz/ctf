...
func TestInfrastructureDoesNotOwnDockerSandboxExecution(t *testing.T) {
	t.Parallel()
...
func TestSubmissionFlowDoesNotDependOnGlobalSubmissionDTO(t *testing.T) {
	...
	files := []string{
		...
		filepath.Join("application", "commands", "submission_submit.go"),
		filepath.Join("application", "commands", "submission_incorrect_submit.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestContestCreateUpdateFlowDoesNotDependOnGlobalContestDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("api", "http", "handler.go"),
		filepath.Join("application", "commands", "contest_create_commands.go"),
		filepath.Join("application", "commands", "contest_update_commands.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestChallengeAddFlowDoesNotDependOnGlobalContestChallengeDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("api", "http", "challenge_handler.go"),
		filepath.Join("application", "commands", "challenge_add_commands.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestParticipationAnnouncementFlowDoesNotDependOnGlobalContestAnnouncementDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("api", "http", "participation_handler.go"),
		filepath.Join("application", "commands", "participation_announcement_commands.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestContestAWDServiceCreateFlowDoesNotDependOnGlobalContestAWDServiceDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("application", "commands", "contest_awd_service_service.go"),
		filepath.Join("application", "commands", "response_mappers.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestAWDCommandResponseMappersDoNotDependOnGlobalAWDDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("application", "commands", "response_mapper_goverter.go"),
		filepath.Join("application", "commands", "response_mapper_goverter_gen.go"),
		filepath.Join("application", "commands", "awd_response_mappers.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestAWDServiceManageHandlerFlowDoesNotDependOnGlobalContestAWDServiceDTO(t *testing.T) {
	t.Parallel()

	files := []string{
		filepath.Join("api", "http", "awd_service_manage_handler.go"),
		filepath.Join("api", "http", "request_mapper_awd_service_support.go"),
	}
	for _, file := range files {
		assertFileDoesNotImport(t, file, "ctf-platform/internal/dto")
	}
...
func TestContestTestsDoNotDependOnGlobalModelOrChallengeEntity(t *testing.T) {
	t.Parallel()
	...
	patterns := []string{
		...
		filepath.Join("application", "jobs", "*_test.go"),
