...
func (s *service) Register(ctx context.Context, req RegisterInput) (*LoginResp, *authcontracts.Session, error) {
	logctx.Info(ctx, s.log, "auth_register_attempt", zap.String("username", req.Username))
	...
	if err := user.SetPassword(req.Password); err != nil {
		logctx.Error(ctx, s.log, "auth_register_password_hash_failed", zap.String("username", req.Username), zap.Error(err))
	...
	if err := s.users.Create(ctx, user); err != nil {
		...
		case errors.Is(err, identitycontracts.ErrUsernameExists):
			logctx.Warn(ctx, s.log, "auth_register_failed_username_exists", zap.String("username", req.Username))
			return nil, nil, identitycontracts.ErrDuplicateUsername
		case errors.Is(err, identitycontracts.ErrEmailExists):
			logctx.Warn(ctx, s.log, "auth_register_failed_email_exists", zap.String("username", req.Username), zap.String("email", req.Email))
			return nil, nil, identitycontracts.ErrDuplicateEmail
		case errors.Is(err, identitycontracts.ErrRoleNotFound):
			logctx.Error(ctx, s.log, "auth_register_failed_role_missing", zap.String("username", req.Username), zap.String("role", user.Role))
			return nil, nil, apperror.ErrInternal.WithCause(err)
		default:
			logctx.Error(ctx, s.log, "auth_register_failed", zap.String("username", req.Username), zap.Error(err))
			return nil, nil, apperror.ErrInternal.WithCause(err)
		}
	}

	logctx.Info(ctx, s.log, "auth_register_succeeded", zap.String("username", user.Username), zap.Int64("user_id", user.ID))
...
func (s *service) Login(ctx context.Context, req LoginInput) (*LoginResp, *authcontracts.Session, error) {
	logctx.Info(ctx, s.log, "auth_login_attempt", zap.String("username", req.Username))

	user, err := s.users.FindByUsername(ctx, req.Username)
	if err != nil {
		if errors.Is(err, identitycontracts.ErrUserNotFound) {
			logctx.Warn(ctx, s.log, "auth_login_failed_user_not_found", zap.String("username", req.Username))
			return nil, nil, authcontracts.ErrInvalidCredentials
		}
		logctx.Error(ctx, s.log, "auth_login_failed_lookup", zap.String("username", req.Username), zap.Error(err))
		return nil, nil, apperror.ErrInternal.WithCause(err)
	}

	if user.Status == identitycontracts.UserStatusBanned {
		logctx.Warn(ctx, s.log, "auth_login_failed_account_disabled", zap.String("username", req.Username), zap.Int64("user_id", user.ID))
	...
	if user.Status == identitycontracts.UserStatusLocked {
		if user.LockedUntil == nil || time.Now().UTC().Before(*user.LockedUntil) {
			logctx.Warn(ctx, s.log, "auth_login_failed_account_locked", zap.String("username", req.Username), zap.Int64("user_id", user.ID))
		...
		if err := s.resetLoginTracking(ctx, user, identitycontracts.UserStatusActive); err != nil {
			logctx.Error(ctx, s.log, "auth_login_failed_unlock_expired_lock", zap.String("username", req.Username), zap.Int64("user_id", user.ID), zap.Error(err))
			return nil, nil, apperror.ErrInternal.WithCause(err)
		}
	}
	...
	if !s.ValidatePassword(user, req.Password) {
		...
		if updateErr != nil {
			logctx.Error(ctx, s.log, "auth_login_failed_record_attempt", zap.String("username", req.Username), zap.Int64("user_id", user.ID), zap.Error(updateErr))
			return nil, nil, apperror.ErrInternal.WithCause(updateErr)
		}
		logctx.Warn(ctx, s.log, "auth_login_failed_invalid_password", zap.String("username", req.Username), zap.Int64("user_id", user.ID), zap.Bool("locked", locked))
		if locked {
	...
	if user.FailedLoginAttempts > 0 || user.LockedUntil != nil || user.Status == identitycontracts.UserStatusLocked {
		...
		if err := s.resetLoginTracking(ctx, user, nextStatus); err != nil {
			logctx.Error(ctx, s.log, "auth_login_failed_reset_attempts", zap.String("username", req.Username), zap.Int64("user_id", user.ID), zap.Error(err))
			return nil, nil, apperror.ErrInternal.WithCause(err)
		}
	}

	logctx.Info(ctx, s.log, "auth_login_succeeded", zap.String("username", user.Username), zap.Int64("user_id", user.ID))
	return s.issueLoginResp(ctx, user)
}
...
func (s *service) issueLoginResp(ctx context.Context, user *identitycontracts.User) (*LoginResp, *authcontracts.Session, error) {
	...
	if err != nil {
		logctx.Error(ctx, s.log, "auth_create_session_failed", zap.String("username", user.Username), zap.Int64("user_id", user.ID), zap.Error(err))
		return nil, nil, apperror.ErrInternal.WithCause(err)
	}
