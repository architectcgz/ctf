...
func TestRootPackageKeepsNoConcreteGoFiles(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("glob auth root files: %v", err)
	}
	...
	if len(nonTestFiles) != 0 {
		t.Fatalf("auth root package should keep no non-test go files, got %v", nonTestFiles)
	}
}
...
func TestAPIHTTPDoesNotDependOnInfrastructure(t *testing.T) {
	...
	for _, file := range files {
		...
		assertFileDoesNotImport(t, file, "ctf-platform/internal/module/auth/infrastructure")
	}
}
...
func TestCommandsDoNotDependOnAPIHTTPOrInfrastructure(t *testing.T) {
	...
	for _, file := range files {
		...
		assertFileDoesNotImport(t, file, "ctf-platform/internal/module/auth/api/http")
		assertFileDoesNotImport(t, file, "ctf-platform/internal/module/auth/infrastructure")
...
func TestAPIHTTPResponseMapperUsesGoverterDelegationOnly(t *testing.T) {
	t.Parallel()

	mapperFile := filepath.Join("api", "http", "response_mapper.go")
	assertFileContains(t, mapperFile, "type authHTTPResponseMapper interface")
	assertFileContains(t, mapperFile, "var authResponseMapper authHTTPResponseMapper")

	assertMapperDelegatesOnly(t, mapperFile, "toAuthUser", "authResponseMapper", "ToAuthUserPtr")
	assertMapperDelegatesOnly(t, mapperFile, "toLoginResp", "authResponseMapper", "ToLoginRespPtr")
	assertMapperDelegatesOnly(t, mapperFile, "toCASStatusResp", "authResponseMapper", "ToCASStatusRespPtr")
	assertMapperDelegatesOnly(t, mapperFile, "toCASLoginResp", "authResponseMapper", "ToCASLoginRespPtr")
}
