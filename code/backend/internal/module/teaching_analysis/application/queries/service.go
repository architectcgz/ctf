...
type QueryService struct {
	...
	repo       teachingAnalysisRepository
	pagination config.PaginationConfig
}

type teachingAnalysisRepository interface {
...
func (s *QueryService) ListClasses(
	...
	if requesterRole == identitycontracts.RoleAdmin {
		...
		return nonNilSlice(teachingAnalysisMapper.ToClassItems(items)), total, page, size, nil
	}
...
func (s *QueryService) ListStudents(
	...
	return nonNilSlice(teachingAnalysisMapper.ToStudentItems(items)), total, page, size, nil
}
...
func (s *QueryService) ListClassStudents(ctx context.Context, requesterID int64, requesterRole, className string, query *TeacherStudentListInput) ([]TeacherStudentItem, error) {
	...
	return nonNilSlice(teachingAnalysisMapper.ToStudentItems(items)), nil
}
