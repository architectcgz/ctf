...
func (s *Service) resolveContestChallengeInstanceScope(ctx context.Context, userID, contestID, challengeID int64) (practiceports.InstanceScope, error) {
	...
	if scope.ContestMode == practiceports.ContestModeAWD {
		return practiceports.InstanceScope{}, apperror.ErrInvalidParams.WithCause(
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestChallengeNotFound) {
			return practiceports.InstanceScope{}, contestcontracts.ErrChallengeNotInContest
		}
		return practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	if !contestChallenge.IsVisible {
		return practiceports.InstanceScope{}, contestcontracts.ErrContestChallengeVisible
	}
	return scope, nil
}
...
func (s *Service) resolveContestAWDServiceInstanceScope(ctx context.Context, userID, contestID, serviceID int64) (int64, practiceports.InstanceScope, error) {
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestAWDServiceNotFound) {
			return 0, practiceports.InstanceScope{}, contestcontracts.ErrChallengeNotInContest
		}
		return 0, practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	if subject == nil {
		return 0, practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(fmt.Errorf("practice contest awd runtime subject is nil"))
	}
	if !subject.Visible {
		return 0, practiceports.InstanceScope{}, contestcontracts.ErrContestChallengeVisible
	}
...
func (s *Service) resolveAdminContestAWDServiceInstanceScope(ctx context.Context, contestID, teamID, serviceID int64) (int64, int64, practiceports.InstanceScope, error) {
	...
	default:
		if contest.Status == practiceports.ContestStatusEnded {
			return 0, 0, practiceports.InstanceScope{}, contestcontracts.ErrContestEnded
		}
		return 0, 0, practiceports.InstanceScope{}, contestcontracts.ErrContestNotRunning
	}
...
func (s *Service) resolveAdminContestAWDServiceInstanceScopeWithContest(ctx context.Context, contest *practiceports.ContestRecord, contestID, teamID, serviceID int64) (int64, int64, practiceports.InstanceScope, error) {
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestTeamNotFound) {
			return 0, 0, practiceports.InstanceScope{}, contestcontracts.ErrTeamNotFound
		}
		return 0, 0, practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	if team.CaptainID <= 0 {
		return 0, 0, practiceports.InstanceScope{}, apperror.ErrInvalidParams.WithCause(errors.New("队伍缺少队长用户"))
	}

	subject, err := s.contestScope.FindContestAWDServiceRuntimeSubject(ctx, contestID, serviceID)
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestAWDServiceNotFound) {
			return 0, 0, practiceports.InstanceScope{}, contestcontracts.ErrChallengeNotInContest
		}
		return 0, 0, practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	if subject == nil {
		return 0, 0, practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(fmt.Errorf("practice contest awd runtime subject is nil"))
	}
	if !subject.Visible {
		return 0, 0, practiceports.InstanceScope{}, contestcontracts.ErrContestChallengeVisible
	}
...
func (s *Service) loadRuntimeSubjectWithScope(ctx context.Context, scope practiceports.InstanceScope, challengeID int64) (*practiceentity.Challenge, *practiceports.RuntimeChallengeTopology, error) {
	...
	if s.runtimeSubject == nil {
		return nil, nil, apperror.ErrInternal.WithCause(fmt.Errorf("practice runtime subject repository is nil"))
	}
	chal, err := s.runtimeSubject.FindByID(ctx, challengeID)
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeChallengeNotFound) {
			return nil, nil, challengecontracts.ErrChallengeNotFound
		}
	...
	if err != nil && !errors.Is(err, practiceports.ErrPracticeChallengeTopologyNotFound) {
		return nil, nil, instancecontracts.ErrContainerCreateFailed.WithCause(err)
	}
...
func (s *Service) loadContestAWDServiceRuntimeSubject(ctx context.Context, contestID, serviceID int64) (*practiceentity.Challenge, *practiceports.RuntimeChallengeTopology, error) {
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestAWDServiceNotFound) {
			return nil, nil, contestcontracts.ErrChallengeNotInContest
		}
		return nil, nil, apperror.ErrInternal.WithCause(err)
	}
	if subject == nil {
		return nil, nil, apperror.ErrInternal.WithCause(fmt.Errorf("practice contest awd runtime subject is nil"))
	}
...
func (s *Service) resolveContestBaseInstanceScope(ctx context.Context, userID, contestID int64) (practiceports.InstanceScope, error) {
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestNotFound) {
			return practiceports.InstanceScope{}, contestcontracts.ErrContestNotFound
		}
		return practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	switch contest.Status {
	...
	default:
		if contest.Status == practiceports.ContestStatusEnded {
			return practiceports.InstanceScope{}, contestcontracts.ErrContestEnded
		}
		return practiceports.InstanceScope{}, contestcontracts.ErrContestNotRunning
	}
	...
	if err != nil {
		if errors.Is(err, practiceports.ErrPracticeContestRegistrationNotFound) {
			return practiceports.InstanceScope{}, contestcontracts.ErrNotRegistered
		}
		return practiceports.InstanceScope{}, apperror.ErrInternal.WithCause(err)
	}
	...
	case practiceports.ContestRegistrationStatusPending:
		return practiceports.InstanceScope{}, contestcontracts.ErrContestRegistrationPending
	default:
		return practiceports.InstanceScope{}, contestcontracts.ErrRegistrationNotApproved
	}
