-- Submission-only schema snapshot.
-- Generated from the current migration chain in code/backend/migrations.
-- Do not use this file as the runtime migration source of truth.
--
...
CREATE TABLE public.awd_scope_controls (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_awd_scope_controls_control_type CHECK (((control_type)::text = ANY ((ARRAY['retired'::character varying, 'service_disabled'::character varying, 'desired_reconcile_suppressed'::character varying])::text[]))),
    CONSTRAINT chk_awd_scope_controls_scope_type CHECK (((scope_type)::text = ANY ((ARRAY['team'::character varying, 'team_service'::character varying])::text[]))),
...
--
-- Name: idx_instances_node_id; Type: INDEX; Schema: public; Owner: -
--
...
CREATE UNIQUE INDEX uk_awd_service_operations_active ON public.awd_service_operations USING btree (contest_id, team_id, service_id) WHERE ((status)::text = ANY ((ARRAY['requested'::character varying, 'provisioning'::character varying, 'recovering'::character varying])::text[]));
...
ALTER TABLE ONLY public.instances
    ADD CONSTRAINT instances_node_id_fkey FOREIGN KEY (node_id) REFERENCES public.runtime_nodes(id);
