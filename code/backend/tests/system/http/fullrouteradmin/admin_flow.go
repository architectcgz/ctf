...
func VerifyAdminCanToggleAWDControlsAndSeeOrchestrationState(
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, resp *practicecommands.AdminAWDScopeControlResp) {
				t.Helper()
				if !resp.Enabled || resp.ControlType != contestcontracts.AWDScopeControlTypeRetired || resp.TeamID != target.TeamID {
		...
		},
		{
			...
			assert: func(t *testing.T, resp *practicecommands.AdminAWDScopeControlResp) {
				t.Helper()
				if !resp.Enabled || resp.ControlType != contestcontracts.AWDScopeControlTypeServiceDisabled || resp.ServiceID == nil || *resp.ServiceID != target.ServiceID {
	...
	for _, controlType := range []string{
		contestcontracts.AWDScopeControlTypeRetired,
		contestcontracts.AWDScopeControlTypeServiceDisabled,
		contestcontracts.AWDScopeControlTypeDesiredReconcileSuppressed,
	} {
		if !seen[controlType] {
