...
| 3. `contest` Phase 1 | completed | 已完成 `api/http`、`application`、`infrastructure` 物理分层；composition/测试已切到新目录，根包旧 `handler / repository / service / scoreboard / scoring` 已物理删除 |
...
- `contest` 根包不再保留 `New*Handler / New*Service / New*Repository / New*ScoreboardService` 兼容入口
