...
def main() -> None:
    ...
    proc = start(root / 'docker' / 'app.py', 18103)
    try:
        text = urllib.request.urlopen('http://127.0.0.1:18103/download/briefing.md').read().decode('utf-8')
        bits = []
        for ch in text:
            if ch == '\u200b':
                bits.append('0')
        ...
        code = ''.join(out)
        body = urllib.request.urlopen(f'http://127.0.0.1:18103/redeem?code={urllib.parse.quote(code)}').read().decode()
