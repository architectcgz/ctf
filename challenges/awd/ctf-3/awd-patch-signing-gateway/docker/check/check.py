...
BASE = sys.argv[1].rstrip("/") if len(sys.argv) > 1 else "http://127.0.0.1:18135"
...
status, body = get("/")
expect(status == 200 and "Patch Signing Gateway" in body, "index page missing")
status, body = get("/channels/demo")
expect(status == 200 and "stable" in body, "demo channel is unavailable")
flag = "flag{local_patch_signing_gateway_check}"
...
status, body = with_retry(lambda: get("/bundle?channel=stable&signer_role=release-manager"))
expect(status == 200 and flag in body, "capability-header exploit did not recover the current flag")
print("ok awd-patch-signing-gateway")
