...
- 本轮专项审查起手时暴露出的高收益问题，已经完成当前测试门禁层面的收口：
  ...
  - 拓扑页的 `TD-1` 也继续从“结构拆分”推进到“样式 owner 收口”：challenge header / workbench / canvas 的样式已回收到 `TopologyChallengeWorkspaceHeader.vue`、`TopologyChallengeWorkbench.vue`、`TopologyCanvasWorkspaceSection.vue`，父页只保留主题变量、模式容器样式和加载 / 空状态分支。
  - `ContestDetail.vue` 的 `TD-1` 也继续从“大块 tab 模板”推进到“剩余 shell owner 收口”：公告 section、队伍 section 和队伍对话框壳已回收到 `ContestAnnouncementsWorkspaceSection.vue`、`ContestTeamWorkspaceSection.vue`、`ContestTeamDialogs.vue`，父页继续只保留 route page 组合 owner。
  - `InstanceList.vue` 的 `TD-1` 也已从“route view 混合 owner / 模板 / 样式”推进到“页面壳 owner 收口”：页面壳、概况卡片、实例目录、过期提醒弹层和局部样式已回收到 `InstanceListWorkspaceShell.vue`，父页从 `493` 行降到 `69` 行，继续只保留 `useInstanceListPage()` 的数据装配与实例动作 owner、`useInstanceWarningFocus()` 的提醒焦点 owner，以及关闭按钮 ref owner。
...
- 已完成：
  - `TD-1` 拓扑页第八个切片完成：模板库模式与挑战模式共用的“入口节点”卡片抽到 `TopologyEntryNodeSection.vue`，父页不再继续内联 `draft.entry_node_key` 选择器和删除拓扑按钮布局。
  - 父组件继续持有 `draft.entry_node_key`、`saving`、`topology` 和删除拓扑动作；新组件只通过 `props + emit(updateEntryNodeKey / deleteTopology)` 承接局部表单和可选删除按钮，没有吸入拓扑草稿 owner 或远端删除逻辑。
  - `ChallengeTopologyStudio.test.ts` 与 `sharedThemeTokenAdoption.test.ts` 已同步补新组件护栏，继续按“父页组合 owner + 子组件承接稳定卡片模板”的方式防回归。
- 本轮涉及文件：
  - `code/frontend/src/components/platform/topology/ChallengeTopologyStudioPage.vue`
...
- 已执行：
  - `npm run test:run -- src/views/platform/__tests__/ChallengeTopologyStudio.test.ts src/views/__tests__/sharedThemeTokenAdoption.test.ts src/views/__tests__/asyncChunkBoundaries.test.ts`（3 个测试文件，19 个测试）
  - `npm run typecheck`
  - `git diff --check`
...
- 已完成：
  - `TD-1` 拓扑页第十个切片完成：模板库模式和 challenge 模式共用的“图形画布”工作区抽到 `TopologyCanvasWorkspaceSection.vue`，父页不再继续内联大段画布按钮、模式提示、校验 banner、canvas board 和 quick editor 组合模板。
  - 父组件继续持有 `interactionMode`、selected canvas state、`draftValidationIssues`、`canvasGraph` 和所有画布更新动作；新组件只通过 `variant + props + emits` 承接画布工作区模板，没有吸入 route/query、远端请求、保存导出动作或第二份 canvas owner。
  - `ChallengeTopologyStudio.test.ts`、`sharedThemeTokenAdoption.test.ts` 与 `asyncChunkBoundaries.test.ts` 已同步改成检查新组件，继续按“父页组合 owner + 子组件承接稳定画布壳”的方式防回归。
- 本轮涉及文件：
  - `code/frontend/src/components/platform/topology/ChallengeTopologyStudioPage.vue`
...
- 已执行：
  - `npm run test:run -- src/views/challenges/__tests__/ChallengeDetail.test.ts src/views/challenges/__tests__/challengeDetailPanelExtraction.test.ts src/views/challenges/__tests__/challengeDetailSharedShell.test.ts src/views/__tests__/pageTabsStyles.test.ts`（4 个测试文件，51 个测试）
  - `npm run typecheck`
  - `git diff --check`
...
- 已完成：
  - `TD-1` `ScoreboardView.vue` 的 workspace shell owner 已收口，顶部 tab rail、contest / points 两块 workspace 面板和局部样式统一抽到 `components/scoreboard/ScoreboardWorkspaceShell.vue`。
  - 父页继续只保留 `useScoreboardRoutePage()` 的 route tab owner、`useScoreboardView()` 的筛选 / 分页 / 刷新 / 数据 owner，以及 contest directory presentation 装配；新 shell 只承接稳定的布局模板和事件转发，没有吸入第二份 route/query 或请求 owner。
  - `ScoreboardView.vue` 本体行数已从 `591` 行降到 `108` 行，已低于前端 view 行数护栏；`architectureAllowlist.ts` 已同步移除该页的 oversized allowlist。
  - `ScoreboardView.test.ts`、`pageTabsStyles.test.ts` 和 `journalUserDirectoryStyles.test.ts` 已改成按父页 + workspace shell 组合源码检查，避免后续继续抽壳时把模板回塞父页。
- 本轮涉及文件：
  - `code/frontend/src/views/scoreboard/ScoreboardView.vue`
...
- 已执行：
  - `npm run test:run -- src/views/scoreboard/__tests__/ScoreboardView.test.ts src/views/__tests__/pageTabsStyles.test.ts src/views/__tests__/journalUserDirectoryStyles.test.ts src/views/__tests__/sharedThemeTokenAdoption.test.ts src/views/__tests__/journalEyebrowStyles.test.ts src/views/__tests__/journalUserShellStyles.test.ts`（6 个测试文件，47 个测试）
  - `npm run typecheck`
  - `git diff --check`
...
- 已完成：
  - `TD-1` `UserProfile.vue` 的 workspace shell owner 已收口，页面壳、顶部资料区、摘要区、账号信息区、个人报告区和局部样式统一抽到 `components/profile/UserProfileWorkspaceShell.vue`。
  - 父页继续只保留 `useUserProfilePage()` 的加载、导出、下载和文案 owner；新 shell 只承接稳定的页面模板、局部样式和事件转发，没有吸入第二份请求或导出流程 owner。
  - `UserProfile.vue` 本体行数已从 `719` 行降到 `47` 行，已低于前端 view 行数护栏；`architectureAllowlist.ts` 已同步移除该页的 oversized allowlist。
  - `UserProfile.test.ts` 与相关源码断言测试已改成按父页 + workspace shell 组合源码检查，避免后续继续抽壳时把页面模板和局部样式回塞父页。
- 本轮涉及文件：
  - `code/frontend/src/views/profile/UserProfile.vue`
...
- 已执行：
  ...
  - `npm run test:run -- src/views/profile/__tests__/UserProfile.test.ts src/views/__tests__/surfaceBackground.test.ts src/views/__tests__/rootHeroLayout.test.ts src/views/__tests__/studentRootShellCleanup.test.ts src/views/__tests__/profileJournalButtonStyles.test.ts src/views/__tests__/journalUserShellStyles.test.ts src/views/__tests__/journalUserDirectoryStyles.test.ts src/views/__tests__/journalEyebrowStyles.test.ts src/views/__tests__/profileJournalUtilityStyles.test.ts src/views/__tests__/profileJournalNoteStyles.test.ts src/views/__tests__/sharedThemeTokenAdoption.test.ts src/views/__tests__/workspaceShellStyles.test.ts -t "首屏页面头部应使用共享 workspace-page-header 分隔线结构|应该在共享样式文件中声明 workspace shell 骨架样式|UserProfile|member pages should consume shared hero shell classes instead of duplicating formula|uses a section root that carries the hero background|student root shell cleanup|profile 页面不应继续在 scoped style 中重复声明 journal 按钮基础规则|profile 与 security 顶部概况应显式使用 metric-panel 类，旧共享 CSS 只保留变量桥接|profile 与 security 页顶部也应接入共享 topbar 与 summary 骨架|已切到 workspace overline 的页面不应继续携带旧 eyebrow 根节点修饰类|profile 页面不应继续在 scoped style 中重复声明 tech-font|profile 页面不应继续在局部样式里重写共享的基础 note 规则|个人资料与安全设置页不应回退到浅色状态块"`（14 个测试文件，32 个测试）
  - `npm run typecheck`

## 第九十轮修复进展

- 已完成：
  - `TD-1` `SkillProfile.vue` 的 workspace shell owner 已收口，页面壳、顶部 tab rail、教师选择区、分析/薄弱项/推荐三块内容面板和局部样式统一抽到 `components/profile/SkillProfileWorkspaceShell.vue`。
  - 父页继续只保留 `useSkillProfilePage()` 的加载、推荐跳转、学员切换 owner，以及 `useUrlSyncedTabs<SkillProfileTabKey>()` 的 route tab owner；新 shell 只承接稳定的页面模板、局部样式和事件转发，没有吸入第二份请求或 tab 状态机 owner。
  - `SkillProfile.vue` 本体行数已从 `741` 行降到 `82` 行，已低于前端 view 行数护栏；`architectureAllowlist.ts` 已同步移除该页的 oversized allowlist。
  - `SkillProfile.test.ts` 与相关源码断言测试已改成按父页 + workspace shell 组合源码检查，避免后续继续抽壳时把页面模板和局部样式回塞父页。
- 本轮涉及文件：
  - `code/frontend/src/views/profile/SkillProfile.vue`
...
- 已执行：
  - `npm run test:run -- src/views/profile/__tests__/SkillProfile.test.ts src/views/profile/__tests__/skillProfileTabsAdoption.test.ts`（2 个测试文件，10 个测试）
  - `npm run typecheck`
  - `npm run test:run -- src/__tests__/architectureBoundaries.test.ts src/views/__tests__/routeViewArchitectureBoundary.test.ts src/views/profile/__tests__/SkillProfile.test.ts src/views/profile/__tests__/skillProfileTabsAdoption.test.ts src/views/__tests__/rootHeroLayout.test.ts src/views/__tests__/studentRootShellCleanup.test.ts src/views/__tests__/profileJournalButtonStyles.test.ts src/views/__tests__/profileJournalUtilityStyles.test.ts src/views/__tests__/profileJournalNoteStyles.test.ts src/views/__tests__/journalUserShellStyles.test.ts src/views/__tests__/journalEyebrowStyles.test.ts src/views/__tests__/pageTabsStyles.test.ts src/views/__tests__/sharedThemeTokenAdoption.test.ts`（13 个测试文件，65 个测试）
  - `npm run test:run -- src/views/__tests__/workspaceShellStyles.test.ts src/views/__tests__/workspacePageHeaderStyles.test.ts -t "首屏页面头部应使用共享 workspace-page-header 分隔线结构|带顶部 tab 的页面不应继续在 tab 面板内重复渲染 eyebrow|不应在页面局部重复声明公共标题排版"`（2 个测试文件，2 个测试命中，16 个跳过）
- 备注：
...
- 已完成：
  - `TD-1` `ContestAwdConfig.vue` 的 workspace shell owner 已收口，稳定工作台模板、局部样式和子组件装配统一抽到 `components/platform/contest/ContestAwdConfigWorkspaceShell.vue`。
  - 父页继续只保留 `useContestAwdConfigPage()` 的路由参数、服务切换、页面数据加载、checker 预览、配置保存和草稿 owner；新 shell 只承接稳定展示区、样式和事件转发，没有吸入第二份请求、路由或保存 owner。
  - `ContestAwdConfig.vue` 本体行数已从 `686` 行降到 `94` 行，已低于前端 view 行数护栏；`architectureAllowlist.ts` 已同步移除该页，当前 oversized route view allowlist 已清空。
  - `ContestAwdConfig.test.ts` 已改成按父页 + workspace shell 组合源码检查，并继续锁定 `useContestAwdConfigPage.ts` 通过 `useAwdCheckerPreviewFlow`、`useAwdCheckerSaveFlow`、`useContestAwdConfigDataLoader` 持有主流程，避免后续继续抽壳时把主业务 owner 回塞到 route view 或壳组件。
- 本轮涉及文件：
  ...
  - `code/frontend/src/__tests__/architectureAllowlist.ts`
  - `docs/reviews/frontend/README.md`
  - `docs/reviews/frontend/2026-05-26-contest-awd-config-workspace-shell-review.md`
  - `docs/reviews/frontend/ctf-frontend-audit-20260422.md`
