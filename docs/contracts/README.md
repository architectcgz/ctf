...
- 修改后运行 `python3 tools/sync_openapi_from_contract.py` 生成 bundle。
- review / doctor 自检时运行 `python3 tools/sync_openapi_from_contract.py --check` 或 `bash scripts/check-workflow-governance.sh`，确认 source 与 bundle 没有漂移。
