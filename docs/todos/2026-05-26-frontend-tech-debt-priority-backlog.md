...
- [ ] P1：收口 admin / teacher 结构耦合，优先停止让 `/platform/*` 直接依赖 teacher 视图或 teacher 语义 owner
  ...
  - `2026-05-27` contract naming 进展：题解投稿目录的共享 DTO 已从 `TeacherSubmissionWriteupItemData` 收口到 `WriteupSubmissionItemData`，platform 题解管理与 teacher 学员分析不再继续共用 teacher 前缀 item contract；当前更深层命名残余转到 `TeacherSubmissionWriteupDetailData`、manual review DTO、`TeacherClassItem`、`TeacherAWDReviewContestItemData` 和 `TeacherAttackSessionQuery`。
