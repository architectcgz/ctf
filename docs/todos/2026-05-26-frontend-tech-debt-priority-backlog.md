...
- [ ] P1：把应属于单一 feature 的 page-sized UI 从 `components/**` 继续收口到 `features/*/ui`
  ...
  - `2026-05-28` platform contest form feature UI 进展：`PlatformContestFormPanel.vue` 与 `PlatformContestFormDialog.vue` 已迁入 `features/platform-contests/ui`，`ContestManage.vue` 改为通过 `features/platform-contests` public API 组合 dialog，`ContestOrchestrationPage.vue` 与 `ContestEditWorkspacePanel.vue` 也已切到 feature 内部 panel import；对应的 2 条 `componentFeatureImportAllowlist` 已收掉。当前这一条在 `platform-contests` 线上剩余的低风险候选，开始进一步收敛到 `PlatformContestTable.vue`、`PlatformContestSummaryStrip.vue` 这类仍留在旧 contest 组件目录的展示 UI。
