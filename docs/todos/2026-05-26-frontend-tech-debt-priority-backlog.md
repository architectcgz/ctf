...
- [x] P1：收口 admin / teacher 结构耦合，优先停止让 `/platform/*` 直接依赖 teacher 视图或 teacher 语义 owner
  ...
  - `2026-05-31` contest edit query owner tightening 进展：`useContestEditPage.ts` 已停止直接读取 `window.location.search`，stage query 读取改为复用共享 `useRouteQueryTransport()`；当前 `ContestEdit` 这条线上 route props、route target contract 和 query owner 已经对齐到同一套 shared transport 约束，后续不需要再把这页当成 `platform/contests` 的残留 query owner 例外。
  - `2026-05-31` contest manage panel owner tightening 进展：`ContestManage` 的 `overview/create` panel query 已从 `ContestOrchestrationPage.vue` 收回 `useContestManagePage.ts`，并新增纯 `useContestManagePanelRoute.ts` 承接 query 解析与构建；当前 `platform/contests` 的目录入口在 panel owner 这一层已经对齐到 `page model + shared route transport + 纯 helper` 模式，不再让 UI shell 自己持有 route-aware 状态。
  - `2026-06-01` contest manage page shell content split 进展：`ContestOrchestrationPage.vue` 已继续退回 contest manage shell owner，overview/create 两块内容装配分别拆到 `ContestManageOverviewPanel.vue` 与 `ContestManageCreatePanel.vue`，页面局部样式也提到 `contestOrchestrationPage.css`；`ContestManage.test.ts` 与 `contestAdminUiStrategy.test.ts` 现已改成同时盯父壳和新 panel owner，后续再继续收这页时不需要回到单个超大 SFC 里混改。
  - `2026-06-01` platform contest manage page owner 进展：`PlatformContestManagePage.vue` 已并入 `features/platform/contests/ui`，`ContestManageRoutePage.vue` 现在只渲染 feature page 薄壳，不再直接组合 `useContestManagePage()`、`ContestOrchestrationPage` 与公告抽屉 / 表单弹窗 / readiness override 这组三层 owner；`ContestManage.test.ts` 也已补上 route 薄壳与 feature page owner 的边界断言。
  - `2026-06-01` platform contest operations hub page owner 进展：`PlatformContestOperationsHubPage.vue` 已并入 `features/platform/contests/ui`，`ContestOperationsHubRoutePage.vue` 现在只渲染 feature page 薄壳，不再直接组合 `useContestOperationsHubPage()` 与 hero/workspace panel；`ContestOperationsHub.test.ts` 也已补上 route 薄壳与 feature page owner 的边界断言。
  - `2026-06-01` platform contest announcements page owner 进展：`PlatformContestAnnouncementsPage.vue` 已并入 `features/platform/contests/ui`，`ContestAnnouncementsRoutePage.vue` 现在只保留 `contestId` route prop 并渲染 feature page，不再直接组合 `useContestAnnouncementsPage()` 与公告 topbar/workspace panel；`ContestAnnouncements.test.ts` 也已补上 route 薄壳与 feature page owner 的边界断言。
  - `2026-06-01` platform contest operations page owner 进展：`PlatformContestOperationsPage.vue` 已并入 `features/platform/contests/ui`，`ContestOperationsRoutePage.vue` 现在只保留 `contestId` route prop 并渲染 feature page，不再直接组合 `useContestOperationsPage()`、`AWDOperationsPanel` 与服务告警插槽；`ContestOperations.test.ts` 也已补上 route 薄壳与 feature page owner 的边界断言。
  - `2026-06-01` platform contest edit page owner 进展：`PlatformContestEditPage.vue` 已并入 `features/platform/contests/ui`，`ContestEditRoutePage.vue` 现在只保留 `contestId` route prop 并渲染 feature page，不再直接组合 `useContestEditPage()`、`ContestEditTopbarPanel`、`ContestWorkbenchStageTabs` 与 `ContestEditWorkspacePanel`；`ContestEdit.test.ts` 也已补上 route 薄壳与 feature page owner 的边界断言。
  - `2026-06-01` platform contests route page owner 收口：`manage / operations hub / announcements / operations / edit` 这 5 张 route page 已全部退回 feature public API 薄壳；继续复查后，`ContestAwdConfigRoutePage.vue` 属于 `features/contest-awd-config` 的独立 page shell，`ContestProjectorRoutePage.vue` 属于 `features/contest-projector` 的独立 page model + shell 组合，不再归入这条 `features/platform/contests/ui` 薄壳迁移线。后续这组如果还有结构债，应该分别按 AWD config 和 projector 自己的 feature 边界处理，而不是继续当成 `platform/contests` route owner 残片。
...
- [x] P1：把应属于单一 feature 的 page-sized UI 从 `components/**` 继续收口到 `features/*/ui`
  ...
  - `2026-05-29` class management route target 进展：`usePlatformClassManagementPage.ts` 与教师班级目录 page model 都已去掉 `vue-router`，分别改为暴露本地班级目录 route target contract；平台班级目录的“查看班级”以及教师班级目录的“教学概览 / 进入班级”现在直接通过 `RouterLink` 消费这些目标路由，教师端“导出班级报告”弹窗 owner 保持不变。`ClassManage.test.ts` 与 `ClassManagement.test.ts` 也已补上“page model 不再 import vue-router、目录 UI 直接消费 route target”护栏，因此 `featureRouterImportAllowlist` 再从班级目录这一组净收掉 2 条。
  ...
  - `2026-06-01` platform instance page owner 进展：`PlatformInstanceManagementPage.vue` 已并入 `features/platform/instance-management/ui`，平台实例 route page 现在只渲染 feature page 薄壳，不再直接组合 `InstanceManageHeroPanel`、`InstanceManageWorkspacePanel` 和 `usePlatformInstanceManagementPage()`；平台实例目录这一条继续对齐到 teacher/user/class/student 已采用的 feature-page owner 模式。
  - `2026-06-01` teacher instance page model naming 进展：教师实例 feature 内部的 `useInstances()` / `useInstanceManagementPage()` 已收口到 `useTeacherInstanceDirectoryState()` / `useTeacherInstanceManagementPage()`，route page 和护栏测试现在都直接体现 teacher-specific owner，不再继续挂着过泛的实例目录 page-state 命名。
  ...
  - `2026-06-01` route page public api 进展：`PlatformOverviewRoutePage.vue` 与 `TeacherDashboardRoutePage.vue` 现在都只从各自 feature 根入口读取 page component 和 page model，不再直指 `model/use*Page.ts`。同时 `routePageArchitectureBoundary.test.ts` 已补上“route page 不能深导入 feature internal module”的通用护栏，这两条总览 route 的公共出口表达终于与实际 owner 对齐。
  - `2026-05-31` teacher dashboard panel owner tightening 进展：教师概览的 `panel` query 已从 `TeacherDashboardPage.vue` 收回 `useDashboardPage.ts`，并新增纯 `teacherDashboardPanelRoute.ts` 承接 query 解析与构建；UI 壳现在只保留 tab 列表和 `useTabKeyboardNavigation()`，不再自己持有 route-aware panel 状态。
  - `2026-05-31` class students panel owner tightening 进展：班级工作区的 `panel` query 已从 `ClassStudentsPage.vue` 收回 `useClassStudentsPage.ts`，并直接复用共享 `useRouteQueryTabs()` 承接 `overview/trend/students/review/insight/action` 的 query-tab owner；UI 壳现在只保留 tab 展示契约，不再自己持有 route-aware panel 状态。
  - `2026-05-31` student analysis panel owner tightening 进展：学员分析工作区的 `panel` query 已从 `StudentAnalysisPage.vue` 收回 `useStudentAnalysisPage.ts`，并直接复用共享 `useRouteQueryTabs()` 承接 `overview/recommendations/writeups/evidence/timeline` 的 query-tab owner；UI 壳现在只保留 tab 展示契约和键盘导航，不再自己持有 route-aware panel 状态。
  - `2026-06-01` challenge detail workspace panel owner tightening 进展：题目详情工作区的 `panel` query 已从 workspace widget 周边收回 `useChallengeDetailPage.ts`，并直接复用共享 `useRouteQueryTabs()` 承接 `question/solution/records/writeup` 的 query-tab owner；`ChallengeDetailWorkspace.vue` 现在只保留 workspace tab 的键盘焦点导航，route page 也去掉了纯转手的 tab wrapper，不再额外改写 query 状态。
  ...
  - `2026-05-31` skill profile panel owner tightening 进展：能力画像页的 `panel` query 已从 `SkillProfileRoutePage.vue` 收回 `useSkillProfilePage.ts`，并新增纯 `skillProfilePanelRoute.ts` 承接 query 解析与构建；route page 现在只保留 tab 列表和 `useTabKeyboardNavigation()`，不再自己持有 route-aware panel 状态。
