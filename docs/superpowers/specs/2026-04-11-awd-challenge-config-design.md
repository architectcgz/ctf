...
在现有后台 `ContestManage -> AWD 运维视图` 内补上 AWD 题目配置入口，让管理员可以在同一工作台里完成以下动作：

- 查看当前 AWD 赛事已关联题目的 checker 配置
- 新增赛事题目关联
...
- `ContestChallengeResp` 会返回 `awd_checker_type / awd_checker_config / awd_sla_score / awd_defense_score`
- `POST /admin/contests/:id/challenges` 支持新增并写入 AWD 配置
- `PUT /admin/contests/:id/challenges/:cid` 支持更新 AWD 配置

前端目前只做了：

- 读取 `listAdminContestChallenges`
- 在 AWD 运维页里把 `challengeLinks` 当成“题目标题映射”

因此真正缺口不在后端，而在前端没有把这组 AWD service 配置做成可操作界面。
...
这轮在 `AWDOperationsPanel` 内增加局部二级 tab，把现有 inspector 留在 `轮次态势`，新增一个 `题目配置` 面板专门处理 contest challenge 的 AWD service 配置。
...
1. `AdminContestChallengeData` 接入 AWD 配置字段
2. `api/admin.ts` 新增：
   - `createAdminContestChallenge`
   - `updateAdminContestChallenge`
3. `useAdminContestAWD` 增加：
   - 题库列表加载
   - 新增/更新 contest challenge
   - 变更后刷新 `challengeLinks`
...
1. API 契约
   - 管理端 contest challenge 列表能归一化 AWD 配置字段
   - 新增和更新接口按后端当前契约发送字段
2. 运维页行为
   - AWD 运维页可切换到题目配置面板
   - 可打开新增与编辑对话框
...
1. 管理员在 AWD 运维页内即可查看每个 contest challenge 的 checker 配置
2. 管理员可以新增一个赛事题目关联并写入 AWD 配置
