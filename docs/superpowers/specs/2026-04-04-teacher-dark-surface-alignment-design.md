...
3. `/teacher/reports` 使用 `report-*` 局部样式，和教师端列表/管理页的低对比暗面存在割裂
...
- 将 `report-*` 样式收敛到教师端低对比暗色体系
- 统一导出表单、预览区、任务状态卡、空态和按钮层级
- 补齐 `ElDialog`、`input`、可能的 `textarea` 或 `AppCard` 内层亮底问题
- 保留报告页稍正式的版式气质，但不再脱离教师端视觉体系
...
- `code/frontend/src/views/teacher/ReportExport.vue`
