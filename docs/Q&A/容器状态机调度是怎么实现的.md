...
如果 API 以多副本部署，这个 loop 不会让所有副本同时干活。每轮开始前会先竞争 Redis 调度锁，只有拿到 `container.scheduler.lock_ttl` 租约并持续续租的那个副本，才有权执行这两件事。
...
这里的“这一轮”是 leader-owner 视角的一轮，不是每个 API 副本各算一次。多副本部署时，锁把这份全局容量判断收口成单点 owner，避免多个副本基于同一份旧计数同时 claim 多条实例。
