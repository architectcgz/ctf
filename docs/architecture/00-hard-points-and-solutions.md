...
- 不把 refresh token 暴露给前端：refresh token 由后端写 HttpOnly Cookie；前端只持有短期 access token（或完全由 cookie 驱动）。
...
- T3（认证/RBAC）：refresh token 存储、强制下线、登录失败锁定、越权访问回归测试。
...
`manifest.yml` 最小字段建议（示意，不是最终 schema）：
...
attachments:
  - path: attachments/data.sql
    ...
    sha256: "..."
flag:
  mode: dynamic            # static | dynamic | manual
  # static_value: ""       # 不建议放在包内；如必须，建议导入时单独字段提交并加密存储
runtime:
  type: container          # container | none（纯题/线下题）
  image:
    source: registry       # registry | dockerfile | tar
...
- `ctf/docs/contracts/examples/challenge-pack-v1/web-hello-01/`（包含 `manifest.yml`、`statement.md`、`docker/Dockerfile`）
