...
- 旧的 `/academy/reports` 通用班级报告页不适合承载 AWD 赛事复盘；当前代码已经移除独立报告页入口，班级报告导出由上下文对话框承接
...
- 教师端旧 `/academy/reports` 与 AWD 赛事复盘的对象模型不一致，当前以 `/academy/awd-reviews` 和学生复盘归档页承接
...
- `/academy/reports` 路由
- 旧 `ReportExport` 独立视图不再作为教师工作台页面存在
- 侧边栏中的通用“报告导出”入口
