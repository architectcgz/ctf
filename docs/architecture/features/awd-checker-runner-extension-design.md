...
这份文档描述目标架构与安全边界。当前已实现能力仍只有 `legacy_probe` 与 `http_standard`；`tcp_standard`、`script_checker` 和外置 sandbox runner 需要按计划新增。
...
`script_checker` 必须运行在独立 runner 中，不允许在 API 进程、轮次调度进程或宿主机 shell 中直接执行。
