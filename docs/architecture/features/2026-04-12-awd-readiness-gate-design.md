...
这会让题目配置能力和赛事运维能力之间继续断开，不符合课题里“集中监控与管理”“自动监控攻击流量、统计防守成功次数”的整体口径。
...
AWD 赛事下，每道已关联题目满足以下条件才视为通过：

- 已配置 `awd_checker_type`
...
- 未配置 checker
- 已配置 checker，但当前保存的 `awd_checker_config` 缺少该 checker 的必需字段，或无法被后端正常解析
...
`global_blocking_reasons` 用于承载不属于单题明细的系统级阻塞原因。本轮至少约定：
...
这层服务只依赖当前已保存的 `contest_challenges` 数据，不依赖临时草稿。
