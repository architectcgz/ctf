...
### 方案 A：preview 接口直接写 `contest_awd_services`

做法：

- 每次试跑成功后，preview 接口直接把最近结果写到 `contest_awd_services`
...
- 创建新 service 时，尚不存在可写入的 `contest_awd_services` 行
...
- token 内部绑定：
  ...
  - preview 结果快照
- 新增/更新 service 配置时，前端把最近一次有效 token 一起提交
- 后端只在保存时消费 token，把校验状态和最近结果写入 `contest_awd_services`
...
4. 后端消费 token，把校验状态和最近一次结果写入 `contest_awd_services`
5. AWD service 列表与编辑对话框都能展示最近一次校验信息

## 数据设计

### `contest_awd_services` 校验字段
...
- 编辑已有 service 时，token 对应同一 `service_id`
- 创建新 service 时，token 对应同一 `template_id/challenge_id`
...
- 配置保存仍然继续
- 但不写入新的已验证状态
- 当前 service 的校验状态按普通配置变更规则处理为 `pending / stale`
...
1. 当前草稿试跑结果
   - 继续沿用 `phase6` 的即时结果区
2. 已保存校验状态
   - 编辑已有 service 时，展示当前 service 最近一次已保存校验结果
...
1. 后端命令与查询
   ...
   - 新增 service 时可消费 token 写入 `passed / failed`
   - 编辑 service 修改 checker 配置但不带 token 时，状态会转 `stale`
   - 仅修改分值等非 checker 字段时，原状态保留
   - 管理端列表接口会返回校验状态与最近结果
2. 前端 API
   - preview 响应能归一化 `preview_token`
   - 新增/更新 service 请求能携带 `awd_checker_preview_token`
   - service 列表能归一化校验状态字段
3. 前端交互
   - 试跑成功后保存会带 token
   - 修改 checker 字段后 token 自动失效
   - 列表页可展示 `未验证 / 最近通过 / 最近失败 / 待重新验证`
   - 编辑已有 service 时可看到最近一次已保存校验结果
