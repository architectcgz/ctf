...
- AWD 运维页内有 `题目配置` 面板
- 可新增/编辑 contest AWD service 的 checker 配置
- 后端已经在 `contest_awd_services.runtime_config / score_config` 中保存 `checker_type / checker_config / awd_sla_score / awd_defense_score`
...
1. 从已有 `checker_config` 解析出表单草稿
2. 根据当前表单状态构建最终提交的 `checker_config`
3. 输出字段级校验错误
