...
- WebSocket `close` 事件（非手动关闭）
- `ping/pong` 超时（`60s` 内未收到 `pong`）
- 网络异常导致连接断开

**不触发重连的场景**：

- 手动调用 `disconnect()`
- 鉴权失败（close code `4001` / `4401`，会触发登出）
- 达到最大重连次数

**心跳机制**：

- 连接建立后每 `30s` 发送一次 `ping` 消息
- 收到 `pong` 后重置超时定时器
- 若 `60s` 内未收到 `pong`，主动关闭连接并触发重连
- 心跳定时器在连接关闭或断开时自动清理

**断线提示**：

- 连接状态通过 `status` reactive 变量暴露：`idle` / `connecting` / `open` / `closed` / `error`
- 各业务 composable 可以监听状态变化并显示提示：
  - 通知页：切换为"手动查看"模式
  - 排行榜页：显示"已断开连接，手动刷新"
  - 公告页：降级为轮询模式
...
- 重连过程中 `status` 为 `connecting`
- 业务层可以显示"正在重连..."提示
- 重连成功后自动恢复实时推送
- 达到最大重连次数后 `status` 变为 `error`，由业务层决定是否显示错误或降级
