...
- `code/backend/internal/module/container_runtime/runtime/module.go`、`code/backend/internal/module/container_runtime/infrastructure/`、`code/backend/internal/module/container_runtime/application/{commands/provisioning_service.go,commands/runtime_cleanup_service.go,container_file_service.go,container_stats_service.go,image_runtime_service.go,node_health_service.go}`
  ...
  - 负责：通过 `code/backend/internal/module/container_runtime/infrastructure/resource_pool_repository.go` 管理按 `runtime_node_id` 分片的 `runtime_port_pool` / `runtime_subnet_pool`。端口和 Docker bridge 子网只在同一 runtime node 内唯一，不同 node 可以复用同一个端口号或子网；reservation 使用 `available -> reserved -> bound` 状态推进，清理时释放回 `available`，运行时冲突资源写入 `quarantined` 并保留在对应 node pool 内排查
...
- `code/backend/internal/app/composition/runtime_node_execution_router.go`、`code/backend/internal/module/practice/application/commands/runtime_container_create.go`、`code/backend/internal/module/container_runtime/application/commands/provisioning_service.go`
  ...
  - 负责：把 `runtime_nodes.endpoint` 解释为 API 到 runtime-agent 的控制面地址，把 `runtime_nodes.public_host/access_host` 解释为该 node 发布端口的数据面访问地址。`code/backend/internal/module/container_runtime/contracts/access_url.go` 中的 fallback 顺序是 public：node `public_host` -> global `container.public_host`；access：node `access_host` -> node `public_host` -> global `container.access_host` -> global `container.public_host`
...
- `code/backend/internal/module/practice/application/commands/instance_start_service.go`、`instance_provisioning_scheduler.go`、`runtime_container_create.go`、`awd_desired_runtime_reconciler.go`
  ...
  - 负责：通过 `provisioning_progress.go` 和 `instance/infrastructure.Repository.RecordProvisioningProgress()` 写入当前 `instances.provisioning_stage / provisioning_attempt / last_provisioning_error`，并追加 `instance_provisioning_events`。普通实例遇到可重试的 subnet / host port / node runtime 错误时会记录 `rescheduling`、清理旧 runtime，并保持 `creating` 等待重新调度；AWD service instance 不走普通静默漂移分支，避免破坏 contest placement
...
- 当前运行态核心数据在 `instances`、`runtime_nodes`、`runtime_port_pool`、`runtime_subnet_pool`、`instance_provisioning_events`、`contest_runtime_placements`、`port_allocations`、`network_allocations`、`contest_awd_services`、`awd_service_operations`、`awd_defense_workspaces`，并受 `runtime_details`、`access_url`、`service_id`、`share_scope`、`flag_key_id`、`runtime_node_id`、`provisioning_stage` 等字段约束。
- `runtime_nodes.name` 是跨 API / runtime-agent / 运维排障使用的稳定逻辑身份，`runtime_nodes.id` 是内部主键并通过 `instances.runtime_node_id`、checker metadata、AWD service routing、`contest_runtime_placements.runtime_node_id` 和 router cache 参与执行路由。`runtime_nodes.endpoint` 是控制面地址；`runtime_nodes.public_host/access_host` 是数据面访问地址。runtime-agent Health 响应包含 `node_name` 与 `hostname`；API 显式配置 `runtime_agent.node_name` 后，缺失或不匹配的 remote agent identity 会阻止 remote client 缓存和使用。
...
- node-scoped 资源池事实持久化在 `runtime_port_pool(runtime_node_id, port)` 和 `runtime_subnet_pool(runtime_node_id, subnet)`；`pool_kind` 只区分 `single_container` 与 `topology` 两类动态子网池，不向 HTTP 契约直接暴露 Docker 子网选择细节。旧 `port_allocations/network_allocations` 仍保留给既有 fallback / cleanup 路径，不作为多 node 分配正确性的 owner。
- provisioning 可观测性事实持久化在 `instances.provisioning_stage / provisioning_attempt / last_provisioning_error` 和 append-only `instance_provisioning_events`。API 响应暴露当前阶段、展示消息和 attempt；事件表保留 stage、severity、runtime_node_id 与 `detail`，用于排查同一次启动里的清理、重试和失败原因。
...
- 配置基线由 `code/backend/internal/config/` 提供，包括 `runtime_agent.node_name`、`runtime_agent.server.node_name`、`container.scheduler.*`、`container.runtime_node_health.*`、`container.startup_recovery_lock_ttl`、`container.proxy_ticket_ttl`、`container.registry.*`、`container.network.single_container_*`、`container.network.topology_*`、`contest.awd.scheduler_*`；两套 CIDR 会在启动时做 IPv4、网络地址、掩码范围和 overlap 校验，当前部署口径仍是 runtime-agent node，不是 Swarm / Kubernetes。
...
- runtime node 身份、访问地址、资源池、quarantine、心跳、健康过滤与 failover requeue：`code/backend/internal/config/config_test.go`、`code/backend/internal/module/container_runtime/infrastructure/agentserver/service_test.go`、`code/backend/internal/app/composition/runtime_module_test.go`、`code/backend/internal/module/container_runtime/application/node_health_service_test.go`、`code/backend/internal/module/container_runtime/infrastructure/node_repository_test.go`、`code/backend/internal/module/container_runtime/infrastructure/resource_pool_repository_test.go`、`code/backend/internal/module/container_runtime/application/commands/provisioning_service_quarantine_test.go`、`code/backend/internal/app/composition/runtime_node_execution_router_test.go`、`code/backend/internal/app/composition/runtime_node_failover_wiring_test.go`、`code/backend/internal/module/instance/infrastructure/repository_test.go`、`code/backend/internal/module/instance/application/commands/runtime_maintenance_service_test.go`
...
- ACL 的 cleanup authority 为实例级 chain handle，持久化在 `runtime_details.acl`；runtime startup 会把仍只有 legacy 平铺规则的实例迁成实例级 chain，随后 cleanup 只按 handle 删除。`runtime_details.acl_rules` 仅保留为调试快照，不再参与 cleanup authority。
