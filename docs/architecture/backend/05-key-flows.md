...
sequenceDiagram
    ...
    SS->>PG: 幂等检查: SELECT 1 FROM submissions WHERE user_id=? AND challenge_id=? AND contest_id=? AND result='correct'
    PG-->>SS: 存在/不存在
...
当有新的解题记录产生时，需要**重新计算该题分值**，并**回溯更新所有已解出该题的用户得分**，最终同步到排行榜。
...
sequenceDiagram
    ...
    rect rgb(200, 220, 240)
        ...
        loop 并发(限制10), 对每个实例
            AWD->>Docker: HTTP GET /health 或 TCP dial timeout=5s
            Docker-->>AWD: 200/timeout
        end
        AWD->>PG: INSERT check_results (round_id, team_id, is_alive, latency_ms)
    end
