...
- `code/backend/internal/module/challenge/application/commands/{challenge_service.go,challenge_import_service.go,awd_challenge_import_service.go,challenge_package_revision_service.go}`、`code/backend/internal/module/challenge/runtime/{import_tx_bridge.go,awd_import_tx_bridge.go,package_export_tx_bridge.go}`、`code/backend/internal/module/challenge/infrastructure/challenge_command_repository.go`
  - 负责：由共享 `ChallengeService` 继续承接题目创建、更新、删除、发布自检、题包导入/导出编排、manifest/topology 重写与 errcode 映射；`challenge/ports.ErrChallengeCommandChallengeNotFound`、`ErrChallengePublishCheckJobNotFound`、`ErrChallengeImageNotFound`、`ErrChallengeTopologyNotFound` 与 `ErrChallengeTopologyPackageRevisionNotFound` 统一表达题目、发布自检任务、镜像、拓扑与题包修订 lookup contract；`challenge/ports.ChallengeImportTxRunner`、`AWDChallengeImportTxRunner`、`ChallengePackageExportTxRunner` 及各自 tx store 定义 use-case-oriented 事务边界，`challenge/runtime/*tx_bridge.go` 负责打开事务、组装 tx-only store、复用 raw challenge repo / image repo / image build 能力，再把这些 bridge 与现有 challenge/image/topology adapter 一起注入 command wiring
  - 不负责：让 challenge application surface 继续直接持有 `*gorm.DB`、分支 `gorm.ErrRecordNotFound`、调用 `.Transaction(...)` 或 `clause.OnConflict`，也不把事务 owner 重新塞回 raw repository 全局语义
