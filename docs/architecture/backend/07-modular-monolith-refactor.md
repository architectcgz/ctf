...
- 当前模块边界已经从旧的 `teacher / system / container` 叙事收口到 `teaching_query / ops / runtime`。
...
- **边界口径**：不再把 `teacher`、`system`、`container` 作为当前主模块叙事；当前代码分别收敛为 `teaching_query`、`ops`、`runtime` 物理模块，并在 app 层明确成 `container_runtime + instance` 两个组合视图。
...
| `runtime` | `challenge`、`contest`、`ops`、`instance` | 各模块 `ports`，以及 `instance/ports` | 当前仍是共享容器能力适配层，同时向 challenge / contest / ops 暴露 consumer-side ports；instance-facing repository / proxy ticket store 仍有一部分由 runtime infrastructure 实现，但 runtime ports 不再 re-export instance ports。practice-facing glue 已上移到 `composition.InstanceModule` |
...
- `runtime -> instance` 这条代码级依赖不再通过 `runtime/ports/http.go`、`runtime/ports/metrics.go` re-export `instance/ports`；当前主要剩余在 runtime infrastructure 对 instance-facing repository / proxy ticket store 的实现，以及 app composition / runtime HTTP handler 对 instance contracts 的显式适配。
