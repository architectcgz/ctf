...
- 负责：描述单进程部署下的 Onion 模块划分、业务 owner、查询聚合边界、composition root 和跨模块协作约束。
...
- `code/backend/internal/module/*/{api,application,domain,ports,infrastructure,runtime,contracts}`
  - 负责：按 Onion 依赖方向组织业务 owner 模块和查询聚合模块，确保 `api -> application -> domain`，`infrastructure -> ports`，`runtime` 只做模块内 wiring
...
- `code/backend/internal/module/auth/application/commands/cas_service.go`、`code/backend/internal/module/auth/infrastructure/cas_ticket_validator.go`
  - 负责：由 CAS command service 继续承接 callback 用例编排、validate URL 构造、用户同步和 session 签发；`auth/ports.CASTicketValidator` 与 `auth/infrastructure/cas_ticket_validator.go` 统一承接 CAS validate request、XML principal 解析、用户名校验和 invalid ticket sentinel，`auth/runtime/module.go` 负责把 validator 注入 command service
...
- `code/backend/internal/module/container_runtime/runtime/`、`code/backend/internal/app/composition/instance_module.go`、`code/backend/internal/app/composition/runtime_module.go`
  ...
  - 不负责：继续让 legacy `runtime/runtime.Module` 承担生产装配或旧 import path 转发，或把已经拆回 owner 模块与 app composition 的 runtime state 再集中回旧物理模块

- `code/backend/internal/module/instance/contracts/*.go`
  - 负责：作为实例访问 adapter、`practice` 与其他组合边界消费实例能力的合法落点，定义实例 owner 暴露给外部模块的 command / query / proxy ticket / maintenance service contract
  - 不负责：承载实例业务规则实现，或把 runtime adapter 便利方法反向塞回 instance owner contract

- `code/backend/internal/module/instance/application/commands/startup_runtime_recovery_service.go`、`code/backend/internal/module/instance/ports/ports.go`、`code/backend/internal/module/instance/infrastructure/{platform_runtime_state_store.go,boot_id_reader.go}`、`code/backend/internal/app/composition/instance_module.go`
...
- `code/backend/internal/module/challenge/application/{challengecore,challengecatalog,challengeimport,challengeselfcheck,challengepublishcheck,challengepackageexport}/`、`code/backend/internal/module/challenge/application/commands/{package_delivery_service.go,awd_challenge_import_service.go,image_build_service.go}`、`code/backend/internal/module/challenge/runtime/{import_tx_bridge.go,awd_import_tx_bridge.go,package_export_tx_bridge.go,wiring.go}`、`code/backend/internal/module/challenge/infrastructure/{challenge_command_repository.go,challenge_import_preview_store.go,challenge_attachment_store.go,challenge_package_storage.go,awd_checker_artifact_store.go,docker_image_builder.go,artifact_gc_service.go}`
  - 负责：由 `challengecore.ChallengeService` 只承接普通题创建、更新、删除、发布和目录变更事件；`challengecatalog` 承接发布目录变更事件的最小共享构造；`challengeimport.ChallengeImportService` 承接普通题包 preview/list/get/commit 编排；`challengeselfcheck.ChallengeSelfCheckService` 承接发布前自检、Flag 校验、镜像可用性检查和 runtime probe 请求；`challengepublishcheck.ChallengePublishCheckService` 承接发布检查 job 生命周期、后台轮询、调用 self-check、通过后调用 core publisher 发布并发事件；`challengepackageexport.ChallengePackageExportService` 承接题包导出和导出修订读取。`commands.PackageDeliveryService` 只作为 Jeopardy / AWD 题包交付模式分发入口，`commands.ImageBuildService` 继续承接镜像构建状态机，AWD 导入仍由 `commands.AWDChallengeImportService` 拥有。`challenge/ports.ErrChallengeCommandChallengeNotFound`、`ErrChallengePublishCheckJobNotFound`、`ErrChallengeImageNotFound`、`ErrChallengeTopologyNotFound` 与 `ErrChallengeTopologyPackageRevisionNotFound` 统一表达题目、发布检查任务、镜像、拓扑与题包修订 lookup contract；`challenge/ports.ChallengeImportTxRunner`、`AWDChallengeImportTxRunner`、`ChallengePackageExportTxRunner` 及各自 tx store 定义 use-case-oriented 事务边界，`challenge/runtime/*tx_bridge.go` 负责打开事务、组装 tx-only store、复用 raw challenge repo / image repo / image build 能力；LocalFS/zip 的上传解包、preview record、附件复制、题包 source/export archive、image build source 与 AWD script checker artifact 持久化由 challenge infrastructure storage adapter 实现；Docker CLI process 执行由 `challenge/infrastructure/docker_image_builder.go` 实现，artifact GC 的本地文件扫描/删除由 `challenge/infrastructure/artifact_gc_service.go` 实现
  - 不负责：让 `application/commands` 继续承载普通题 core/import/self-check/publish-check/export service，或让 core `ChallengeService` 继续挂载导入、自检、发布检查或题包导出方法；让 challenge application surface 直接持有 `*gorm.DB`、分支 `gorm.ErrRecordNotFound`、调用 `.Transaction(...)` / `clause.OnConflict`，或直接实现 Docker CLI、LocalFS/env 操作、zip 解包、preview JSON、附件复制、source 复制、checker artifact 持久化、artifact GC 和 export zip 生成
...
- `code/backend/internal/module/practice/application/commands/score_service.go`、`code/backend/internal/module/practice/application/queries/score_service.go`、`code/backend/internal/module/practice/infrastructure/score_state_store.go`、`code/backend/internal/module/practice/infrastructure/score_query_repository.go`
  ...
  - 不负责：让 practice score application surface 继续直接知道 Redis key、Lua compare-and-del、JSON 缓存格式、`ZAdd` 细节或 `gorm.ErrRecordNotFound`

- `code/backend/internal/module/practice/application/commands/{focused_services.go,submission_service.go}`、`code/backend/internal/module/practice/contracts/submission.go`、`code/backend/internal/module/practice/infrastructure/submission_rate_limit_store.go`
  - 负责：由 `SubmissionService` 承接 Flag 提交和个人提交历史 use case；`practice/contracts/submission.go` 承接 handler 与 application 共享的提交响应 DTO；`practice/ports.PracticeFlagSubmitRateLimitStore` 与 `practice/infrastructure/submission_rate_limit_store.go` 统一承接 prefix fallback、计数 key、`Incr` 和首次窗口 `Expire` 细节；`practice/runtime/module.go` 通过 `NewCommandServices` 构造并把 `Submissions` 依赖注入 HTTP handler
  - 不负责：让 submission application surface 直接持有 Redis client，或在 application 层拼接 flag submit rate-limit key；让 HTTP handler 继续依赖一个覆盖所有写路径的 command facade

- `code/backend/internal/module/practice/application/commands/submission_service.go`、`code/backend/internal/module/practice/infrastructure/solved_submission_repository.go`、`code/backend/internal/module/practice/infrastructure/runtime_subject_repository.go`
  - 负责：由 `SubmissionService` 继续承接 challenge 发布状态判定、重复解题分支、动态 flag 判定和 errcode 映射；`practice/ports.PracticeSolvedSubmissionRepository` 与 `practice/ports.PracticeRuntimeSubjectRepository` 统一表达正确提交和 challenge runtime subject 的 not-found contract，`practice/infrastructure/solved_submission_repository.go` 与 `practice/infrastructure/runtime_subject_repository.go` 负责把 raw practice repository / challenge contract 的 `gorm.ErrRecordNotFound` 收口成模块内 sentinel，`practice/runtime/module.go` 负责把 adapter 注入 command services
  - 不负责：让 submission application 继续直接知道 `gorm.ErrRecordNotFound`，或直接依赖 raw repository / challenge raw contract 的 not-found 语义

- `code/backend/internal/module/practice/application/commands/{focused_services.go,instance_start_service.go,instance_provisioning.go,instance_provisioning_scheduler.go,awd_desired_runtime_reconciler.go,awd_scope_control_commands.go,contest_awd_operations.go}`、`code/backend/internal/module/practice/contracts/awd_instance.go`、`code/backend/internal/module/practice/infrastructure/{instance_readiness_probe.go,contest_scope_repository.go,runtime_subject_repository.go}`
  - 负责：由 `InstanceLifecycleService` 承接普通开题、竞赛开题、AWD team/service 启停、编排查询、scope control 与 prewarm use case；由 `RuntimeLifecycleService` 承接后台 provisioning loop、desired AWD reconcile、background task lifecycle；`practice/contracts/awd_instance.go` 承接 handler 与 application 共享的 AWD 实例响应 DTO；`practice/ports.PracticeInstanceReadinessProbe` 与 `practice/infrastructure/instance_readiness_probe.go` 统一承接 access URL 的 HTTP GET、TCP dial、URL 解析和响应体回收细节，`practice/runtime/module.go` 负责把 probe、scope repository 与 runtime subject adapter 注入 command services
  - 不负责：让 practice provisioning surface 继续直接知道 `net/http` client、`url.Parse` 或 `net.Dialer` 细节

- `code/backend/internal/module/practice/application/commands/contest_instance_scope.go`、`code/backend/internal/module/practice/application/commands/contest_awd_operations.go`、`code/backend/internal/module/practice/infrastructure/contest_scope_repository.go`、`code/backend/internal/module/practice/infrastructure/runtime_subject_repository.go`
  - 负责：由 `InstanceLifecycleService` 继续承接 contest 开题 scope、AWD orchestration 查询、可见性检查和 errcode 映射的业务编排；`practice/ports.PracticeContestScopeRepository` 与 `practice/ports.PracticeRuntimeSubjectRepository` 统一表达 contest / challenge runtime subject 的 not-found contract，两个 infrastructure adapter 负责把 raw practice repository 与 challenge contract 的 `gorm.ErrRecordNotFound` 收口成模块内 sentinel，`practice/runtime/module.go` 负责把 adapter 注入 command services
  - 不负责：让 practice application surface 继续直接知道 `gorm.ErrRecordNotFound`，或直接依赖 challenge raw repository 的 not-found 语义

- `code/backend/internal/module/practice/application/commands/{focused_services.go,manual_review_service.go}`、`code/backend/internal/module/practice/infrastructure/manual_review_repository.go`
  - 负责：由 `ManualReviewService` 继续承接教师评阅、班级可见性校验、已解题判定和 errcode 映射的业务编排；`practice/ports.PracticeManualReviewRepository` 与 `practice/ports.PracticeRuntimeSubjectRepository` 统一表达人工评阅提交、已通过提交、教师用户和 challenge runtime subject 的 not-found contract，`practice/infrastructure/manual_review_repository.go` 与 `practice/infrastructure/runtime_subject_repository.go` 负责把 raw practice repository / challenge contract 的 `gorm.ErrRecordNotFound` 收口成模块内 sentinel，`practice/runtime/module.go` 负责把 adapter 注入 `ManualReviewService` 并把 `ManualReviews` 依赖注入 HTTP handler
  - 不负责：让 manual review application 继续直接知道 `gorm.ErrRecordNotFound`，或直接依赖 raw repository 的 not-found 语义；让人工评阅继续挂在覆盖实例、提交和后台任务的单一 service 上
...
- `code/backend/internal/module/ops/application/queries/dashboard_service.go`、`code/backend/internal/module/ops/infrastructure/dashboard_state_store.go`
  - 负责：由 dashboard query 继续承接缓存优先读取、runtime stats 聚合、平均值与告警计算的查询编排；`ops/ports.DashboardStateStore` 与 `ops/infrastructure/dashboard_state_store.go` 统一承接 dashboard cache、在线会话扫描和 session payload JSON 细节，`ops/runtime/module.go` 负责把同一个 state store 注入 dashboard query wiring
  - 不负责：让 dashboard query 继续直接知道 Redis client、cache key、auth session 扫描、`Scan/MGet/Get/Set` 或 JSON 编解码细节

- `code/backend/internal/module/ops/application/commands/notification_service.go`、`code/backend/internal/module/ops/infrastructure/notification_repository.go`
  - 负责：由 notification command 继续承接通知业务编排、错误码映射和 websocket fanout；`ops/ports.ErrNotificationNotFound` 与 `ops/infrastructure/notification_repository.go` 统一承接 ORM not-found 到模块内 repo 契约的映射
  - 不负责：让 notification command 继续直接知道 `gorm.ErrRecordNotFound` 这类 persistence sentinel

- `code/backend/internal/module/assessment/application/commands/profile_service.go`、`code/backend/internal/module/assessment/infrastructure/state_store.go`
  - 负责：由 profile command 继续承接画像增量更新和全量重建的业务编排；`assessment/ports.AssessmentProfileLockStore` 与 `assessment/infrastructure/state_store.go` 统一承接画像锁 key、TTL、`SetNX` 和 safe release 细节，`assessment/runtime/module.go` 负责把 lock store 注入 profile command wiring
  - 不负责：让 profile command 继续直接知道 Redis client、lock key、`SetNX` 或 `Del` 细节

- `code/backend/internal/module/assessment/application/queries/recommendation_service.go`、`code/backend/internal/module/assessment/infrastructure/state_store.go`
  - 负责：由 recommendation query 继续承接默认 limit 缓存命中、推荐生成和事件触发缓存失效的查询编排；`assessment/ports.AssessmentRecommendationCacheStore` 与 `assessment/infrastructure/state_store.go` 统一承接 recommendation cache key、TTL、JSON 编解码和 `Get/Set/Del` 细节
  - 不负责：让 recommendation query 继续直接知道 Redis client、recommendation cache key 或缓存序列化细节

- `code/backend/internal/module/assessment/application/commands/report_service.go`、`code/backend/internal/module/assessment/ports/ports.go`、`code/backend/internal/module/assessment/infrastructure/{report_repository.go,report_output_store.go}`
  - 负责：由 report service 继续承接报表用例编排、权限校验和错误码映射；`assessment/ports.ErrAssessmentReportNotFound`、`assessment/ports.ErrAssessmentContestNotFound` 与 `assessment/infrastructure/report_repository.go` 统一承接 ORM not-found 到模块内 repo 契约的映射；`assessment/ports.ReportOutputStore`、`ErrAssessmentReportOutputNotFound` 与 `ErrAssessmentReportOutputUnsafePath` 统一表达报表输出文件准备和下载路径解析语义，`assessment/infrastructure/report_output_store.go` 负责 LocalFS 目录创建、安全路径检查和文件存在性检查
  - 不负责：让 report service 继续直接知道 `gorm.ErrRecordNotFound` 这类 persistence sentinel，或直接调用 `os.MkdirAll`、`os.Stat`、`filepath.Abs/Clean` 处理报表输出文件
...
flowchart TB
    ...
    subgraph L3["后端服务层（单体 Go API 进程）"]
        ...
        subgraph WriteModels["写模型与运行时模块"]
            ...
            ContainerRuntime["容器运行时模块（container_runtime）<br/>容器能力 / 探针 / 文件访问"]
        end
        subgraph QueryAggregation["查询聚合模块"]
    ...
    class Auth,Identity,Challenge,Instance,Practice,Contest,Assessment,OpsSvc,ContainerRuntime,Teaching service
...
- 业务 owner 模块负责自己的表和状态变更；查询聚合模块只消费 owner 暴露的 query / contract / repository 能力，不直接回写底层表。
...
- legacy runtime 回归守卫：`code/backend/internal/module/instance/architecture_test.go`、`code/backend/internal/module/container_runtime/architecture_test.go`
  - 阻止已退场的 `runtime/runtime` 兼容 module 包、`runtime/application`、`runtime/ports` 与 `runtime/infrastructure` import path 回流
...
- 查询聚合模块可以没有 `domain` 或 `contracts`，但依赖方向仍然向内。
...
这张图描述的是 `internal/app/composition/*.go` 里的进程装配关系，也就是 router/runtime 在启动时把哪些模块或组合视图接在一起。`container_runtime` 在这里既有 `internal/module/container_runtime/runtime` 底层 module builder，也有 app 层的 `ContainerRuntimeModule` 组合视图；`instance` 在这里是 app 层组合视图并连接 `internal/module/instance/*` 业务 owner。
...
flowchart LR
    ...
    Challenge -->|identity contracts| Identity
    Challenge -->|container contracts| ContainerRuntime

    Assessment -->|challenge contracts| Challenge
    Assessment -->|practice contracts| Practice
    Assessment -->|contest contracts| Contest
    Assessment -->|identity contracts| Identity

    Ops -->|token service| Auth
    Ops -->|challenge events| Challenge
    Ops -->|contest realtime events| Contest
    Ops -->|identity contracts| Identity
    Ops -->|practice events| Practice

    Contest -->|token service| Auth
    Contest -->|contracts / ports| Challenge
    Contest -->|container capability ports| ContainerRuntime
    Contest -->|identity contracts| Identity
    ...
    TeachingRM -->|challenge contracts| Challenge
    TeachingRM -->|contest contracts| Contest
    TeachingRM -->|identity contracts| Identity
```
...
| `assessment` | `challenge`、`contest`、`identity`、`practice` | `challenge/contracts`、`contest/contracts`、`identity/contracts`、`practice/contracts` | 画像增量更新和推荐缓存刷新订阅 practice / contest 事件；题目、用户和竞赛事实通过窄 contract / provider 进入本模块；班级报告导出使用 assessment 自有 class insight 端口类型 |
...
- `moduleDependencyBaseline` 已不再包含 `container_runtime -> contest`、`instance -> contest` 与 `assessment -> teaching_query`。checker 执行契约已经迁到 `container_runtime` 中性 `SandboxExecutor`，AWD proxy / defense SSH 准入查询已经按 `contest -> instance/ports` 的方向由 contest 实现并注入 instance；班级报告导出的 class insight 数据由 `assessment/ports.AssessmentClassInsightRepository` 定义消费方类型，再由 app composition adapter 映射 `teaching_query` 仓储结果。
...
| 查询聚合 | 教师端、复盘、统计、跨模块列表查询 | 只读，不拥有状态变更 |
| `events.Bus` | 异步通知、非关键路径广播 | 不能替代关键写路径事务语义 |

### 5.4 共享运行时边界

`container_runtime` 已成为容器运行能力的底层 module builder 和 capability owner；legacy `runtime` 模块已退役。app 层已经把“实例入口”从“容器运行能力”中拆成两个显式组合视图，并把实例 owner use case 直接装到 `internal/module/instance/*`：
...
- `InstanceModule` 现在直接在 composition 边缘组合 `PracticeInstanceRepository`（默认走 `instanceinfra.Repository`，AWD service operation 写路径桥接 contest repo，allocation 释放桥接 container_runtime repo）与 `PracticeRuntimeService`；这层 practice-facing glue 已不再停留在 runtime module 层
...
- runtime-agent protocol、client bridge 与 server bridge 已迁到 `container_runtime/agentcontracts` 与 `container_runtime/infrastructure/agent*`；checker 执行通道使用中性 `SandboxExecutor`，contest 业务 runner 只在 app/composition 与 contest infrastructure 侧适配
...
- runtime 残余状态已按 owner 拆分：runtime node / allocation 归 `container_runtime`，实例清理 / startup recovery Redis 状态 / ACL 迁移读写归 `instance` 与 app composition，AWD workspace / operation / scope control / proxy traffic 归 `contest`。
