...
| `POST /api/v1/auth/mcp-token` | `Handler.IssueMCPToken` | `authcontracts.TokenService`，为外部 agent 签发 MCP Bearer token，并记录 `mcp_token/create` 审计 |
...
| Token service | `auth/infrastructure/token_service.go` | Redis-backed session、JWT、WebSocket ticket、MCP token 校验与签发；MCP token TTL 使用 `auth.mcp_token_ttl`，默认 6h | HTTP 路由权限判断、用户状态事实 |
