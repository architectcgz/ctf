...
- 2026-05-17 的 runtime teacher instance DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/instances` 保持原查询参数、越权校验和 JSON 字段不变，变化仅在内部 owner：教师实例列表 request / response DTO 不再挂在全局 `internal/dto/teacher.go`，其中 HTTP 输入输出收回 `runtime/api/http`，跨模块查询契约收回 `instance/contracts`。
...
- 2026-05-17 的 teaching query teacher timeline DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/students/:id/timeline` 保持原路径、查询参数和 JSON 字段不变，变化仅在内部 owner：教师侧 student timeline response DTO 从全局 `internal/dto` 收回 `teaching_query/application/queries`，handler 继续直接返回该查询快照。
- 2026-05-17 的 teaching query request DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/classes`、`GET /api/v1/teacher/students`、`GET /api/v1/teacher/classes/:name/students`、`GET /api/v1/teacher/classes/:name/{summary,trend,review}`、`GET /api/v1/teacher/students/:id/evidence` 与 `GET /api/v1/teacher/students/:id/attack-sessions` 保持原路径、查询参数和校验口径不变，变化仅在内部 owner：教师侧 class / student / evidence / attack session request DTO 从全局 `internal/dto` 收回 `teaching_query/api/http`，application 层改为消费 `teaching_query/application/queries` 自有 input。
...
- 2026-05-17 的 challenge response DTO localization（image/tag/flag/topology/awd）同样没有新增、删除或重命名外部 HTTP 契约；相关路由保持原路径、状态码与 JSON 字段不变，变化仅在内部 owner：handler 输出改为通过 `challenge/api/http` response mapper 把全局 `dto` 映射为模块内 response DTO。
