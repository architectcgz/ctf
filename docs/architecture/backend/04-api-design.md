...
- `code/backend/internal/interfaces/mcp/handler.go`、`code/backend/internal/app/router.go`
  - 负责：装配 `POST /mcp` JSON-RPC / MCP 入口，外部 agent 通过 OAuth 2.1 Authorization Code + PKCE 获取 `mcp:challenge:read` access token 后调用；当前提供 `initialize`、`tools/list` 与 `tools/call`，第一版工具 `get_current_challenge` 只读当前用户最近的活动实例和对应已发布题目详情；MCP 工具调用按用户走 `rate_limit.mcp`，成功调用写入审计日志
...
- `POST /mcp` 是 MCP JSON-RPC 入口，不使用 `ApiEnvelopeBase`。外部 agent 先通过 `/.well-known/oauth-protected-resource` 与 `/.well-known/oauth-authorization-server` 发现 OAuth 端点，再使用 `/api/v1/oauth/register`、`/api/v1/oauth/authorize` 和 `/api/v1/oauth/token` 完成浏览器授权。`initialize`、`notifications/initialized` 和 `tools/list` 可无 token 调用；`tools/call` 必须使用 `Authorization: Bearer <oauth_access_token>`，并要求 `mcp:challenge:read` scope。未认证的 `tools/call` 返回 HTTP 401、`WWW-Authenticate: Bearer realm="ctf-mcp", resource_metadata=".../.well-known/oauth-protected-resource"` 和 JSON-RPC 错误 `-32001`，`data` 只包含 `auth_method=oauth`、`resource_metadata` 与 `required_scope`。`get_current_challenge` 的 `structuredContent` 返回 `has_current_challenge`、`instance` 和 `challenge`，无活动实例时返回 `has_current_challenge=false`。MCP 工具调用按 `rate_limit.mcp` 做用户级限流，默认 120/min，超限返回 JSON-RPC 错误 `-32002`；成功工具调用记录 `mcp_tool/read` 审计，detail 包含 `client_id` 和 `scope`，不记录 token。
...
- 2026-05-12 的 teacher overview aggregate 新增 `GET /api/v1/teacher/overview`；该接口由 `teaching_analysis` 输出 summary-scope 聚合结果，服务 `/academy/overview`，不再让前端通过默认班级去拼 `classes/:name/*` 的 detail DTO。
...
- 2026-05-12 的 teaching analysis overview query surface phase 4 / slice 2 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/overview` 保持原路径与响应结构，内部 owner 从宽 `teaching_analysis/application/queries.Service` 收口到独立 `OverviewService`，handler 也改为分别依赖 overview 与其余教师查询。
- 2026-05-12 的 teaching analysis class insight query surface phase 4 / slice 3 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/classes/:name/{summary,trend,review}` 保持原路径与响应结构，内部 owner 从剩余宽 `teaching_analysis/application/queries.Service` 收口到独立 `ClassInsightService`，handler 也改为分别依赖目录/学生查询、overview 和 class insight。
- 2026-05-13 的 teaching analysis student review query surface phase 4 / slice 4 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/students/:id/{progress,recommendations,timeline,evidence,attack-sessions}` 保持原路径、响应结构和权限口径不变，内部 owner 从剩余宽 `teaching_analysis/application/queries.Service` 收口到独立 `StudentReviewService`，handler 改为分别依赖目录查询、overview、class insight 和 student review 四类 query owner。
...
- 2026-05-17 的 assessment recommendation contract convergence 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/recommendations` 与 `GET /api/v1/teacher/students/:id/recommendations` 保持原路径、查询参数和响应结构不变，内部 owner 改为由 `assessment/contracts` 输出 recommendation contract，再分别在 `assessment/api/http` 与 `teaching_analysis/application/queries` 映射回现有 DTO。
- 2026-05-17 的 assessment recommendation HTTP DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/recommendations` 保持原路径和 JSON 字段不变，变化仅在内部 owner：学生端 recommendation response DTO 从全局 `internal/dto` 收回 `assessment/api/http`，教师端 recommendation DTO 仍暂留在 `teaching_analysis` 消费边界。
...
- 2026-05-17 的 assessment skill profile command/report DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；能力画像对外仍通过既有 `GET /api/v1/users/me/skill-profile`、`GET /api/v1/users/:id/skill-profile` 与 `GET /api/v1/teacher/students/:id/skill-profile` 暴露，变化仅在内部 owner：`assessment/application/commands` 与 report/export 链改为直接依赖 `assessment/contracts.SkillProfile` / `SkillDimension`，旧的全局 `internal/dto/skill_profile.go` 退出，导出 JSON 字段保持原命名。
- 2026-05-17 的 assessment teacher AWD review request DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/awd/reviews` 与 `GET /api/v1/teacher/awd/reviews/:id` 保持原路径、查询参数和校验口径不变，变化仅在内部 owner：contest list / archive request DTO 从全局 `internal/dto` 收回 `assessment/api/http`，handler 与 request mapper 继续只输出既有 application input。
- 2026-05-17 的 assessment teacher AWD review response DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/awd/reviews`、`GET /api/v1/teacher/awd/reviews/:id` 以及对应 archive/report export 继续沿用既有 JSON 字段与导出结构，变化仅在内部 owner：contest list / archive 响应与导出归档模型从全局 `internal/dto` 收回 `assessment/application/queries`，handler、runtime 包装层与 report/export 链统一消费模块内查询快照。
- 2026-05-17 的 runtime teacher instance DTO localization 与 2026-06-09 的 instance HTTP owner 收口都没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/instances`、实例访问 proxy、AWD target proxy 与 defense SSH 保持原查询参数、越权校验和 JSON 字段不变，变化仅在内部 owner：教师实例列表 request / response DTO 不再挂在全局 `internal/dto/teacher.go`，实例访问相关 HTTP 输入输出统一落在 `instance/api/http`，旧的 `runtime/api/http` 兼容实现已退出。
...
- 2026-05-17 的 teaching analysis teacher timeline DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/students/:id/timeline` 保持原路径、查询参数和 JSON 字段不变，变化仅在内部 owner：教师侧 student timeline response DTO 从全局 `internal/dto` 收回 `teaching_analysis/application/queries`，handler 继续直接返回该查询快照。
- 2026-05-17 的 teaching analysis request DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`GET /api/v1/teacher/classes`、`GET /api/v1/teacher/students`、`GET /api/v1/teacher/classes/:name/students`、`GET /api/v1/teacher/classes/:name/{summary,trend,review}`、`GET /api/v1/teacher/students/:id/evidence` 与 `GET /api/v1/teacher/students/:id/attack-sessions` 保持原路径、查询参数和校验口径不变，变化仅在内部 owner：教师侧 class / student / evidence / attack session request DTO 从全局 `internal/dto` 收回 `teaching_analysis/api/http`，application 层改为消费 `teaching_analysis/application/queries` 自有 input。
...
- 2026-05-17 的 auth DTO localization 同样没有新增、删除或重命名外部 HTTP 契约；`POST /api/v1/auth/register`、`POST /api/v1/auth/login`、`GET /api/v1/auth/profile`、`POST /api/v1/auth/change-password`、`POST /api/v1/auth/ws-ticket`、`GET /api/v1/auth/cas/{status,login,callback}` 保持原路径和 JSON 字段不变，变化仅在内部 owner：auth request/response 与 CAS query output 从全局 `internal/dto` 收回 `auth/api/http` 与 `auth/application/{commands,queries}`。
- 2026-05-17 的 notification DTO localization 同样没有新增、删除或重命名外部 HTTP / WebSocket 契约；`GET /api/v1/notifications`、`PUT /api/v1/notifications/:id/read`、`POST /api/v1/admin/notifications` 与 `/ws/notifications` 保持原路径、查询参数、推送事件类型和 payload 字段不变，变化仅在内部 owner：notification request/query/response 与 audience 常量从全局 `internal/dto` 收回 `ops/api/http` 与 `ops/application/{commands,queries}`。
...
| `POST` | `/api/v1/oauth/register` | * | 动态注册 public OAuth client |
| `GET` | `/api/v1/oauth/authorize` | @ / 浏览器 | OAuth authorization endpoint；未登录时重定向到登录页 |
| `POST` | `/api/v1/oauth/authorize` | @ / 浏览器 | OAuth consent decision endpoint |
| `POST` | `/api/v1/oauth/token` | OAuth client | authorization code / refresh token 交换 |
