...
当前状态（2026-05-13，phase 5 / slices 1-32）：
...
- 当前 `contest` application surface 已完成 submission challenge lookup、AWD preview runtime lookup、报名/进度、team query 与 team command 这五组 GORM concrete 收口。
...
- 当前 `challenge` application surface 已清空 Redis / HTTP concrete allowlist，并继续删掉了 challenge query、image query、flag command/query、AWD challenge command/query、writeup command/query 与 topology command/query 这十条 GORM concrete 例外。
