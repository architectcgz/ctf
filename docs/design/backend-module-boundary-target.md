...
当前状态（2026-05-13，phase 5 / slices 1-28）：
...
- 当前 `contest` application surface 已完成报名/进度、team query 与 team command 这三组 GORM concrete 收口。
...
- 当前 `challenge` application surface 已清空 Redis / HTTP concrete allowlist，并继续删掉了 image query、flag command/query、AWD challenge command/query 与 writeup command/query 这七条 GORM concrete 例外。
