...
这个问题不是某个组件实现错误，而是任务入口缺少强制路由：项目已经有 `harness-router` skill 和 `harness/prompts/harness-router.md`，但根 `AGENTS.md` 没有明确写出“默认先进入 harness”。
...
- 可复用 prompt 继续放在 `harness/prompts/harness-router.md`，入口文件只做导航和硬约束。
