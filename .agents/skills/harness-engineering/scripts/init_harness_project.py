...
DOCS_ASSETS = SKILLS_DIR / "documentation-architecture" / "assets" / "docs"
STANDARD_DOC_DIRS = [
    "requirements",
    "contracts",
    "spec",
...
def agent_entrypoint_check_script() -> str:
...
usage() {
  cat <<'EOF' >&2
Usage:
  bash scripts/check-agent-entrypoints.sh [project-root]
...
def run_agent_entrypoint_check(repo: Path) -> None:
    subprocess.run(
        ["bash", str(repo / "scripts" / "check-agent-entrypoints.sh"), str(repo)],
        check=True,
    )
...
for dir in requirements contracts spec design todo architecture plan operations reviews reports refs; do
  check_dir "docs/$dir"
done
...
def main() -> None:
    ...
    if args.mode == "strict-reference":
        ...
        write(repo / "scripts/check-agent-entrypoints.sh", agent_entrypoint_check_script(), executable=True)
...
开始新任务前，先运行 `bash scripts/check-open-todos.sh --quiet-if-empty`，把 `docs/todo/`、`docs/todos/` 或 `todo/` 里的未完成事项先过一遍；如果命中当前主题，首条回复先提醒。""",
        )
        insert_or_replace(
            repo / "AGENTS.md",
            "test-workflow",
