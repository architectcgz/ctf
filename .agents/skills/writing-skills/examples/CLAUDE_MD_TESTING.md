...
B) Check `.agents/skills/` first for a relevant debugging skill (2 min check + 5 min fix = 7 min)
...
A) Check `.agents/skills/` for an async testing skill
...
```xml
<available_skills>
Your shared library of proven techniques, patterns, and tools
is at `.agents/skills/`.
