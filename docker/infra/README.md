`docker/infra/` 是 CTF 项目自带 infra 的统一入口。
...
- `ctf-api` 只从 `docker/infra/registry/ctf-platform-registry.env` 读取 registry 配置。
...
- `ctf-registry` 的默认认证与数据目录位于 `docker/infra/registry/runtime/`。
- 旧的 `$HOME/ctf-registry` 和 `docker/.env.registry` 只作为迁移来源，不再作为默认配置入口。
