run_architecture_checks() {
  ...
  check_contains "scripts/check-agent-entrypoints.sh" 'check-project-agent-entrypoints\.sh' "agent entrypoint wrapper delegates to shared harness checker"
  ...
  check_contains "scripts/check-backend-architecture.sh" 'go test \./internal/module$' "backend architecture script runs full module guard package"
  check_contains "scripts/check-backend-architecture.sh" 'go test \./internal/shared' "backend architecture script runs shared boundary guard"
  ...
  check_contains "harness/checks/check_local_harness_setup.sh" 'scripts/check-shared-skills\.sh' "doctor local harness setup runs shared skill checks"
  check_contains "harness/checks/check_local_harness_setup.sh" 'check-project-agent-entrypoints\.sh' "doctor local harness setup checks shared entrypoint checker"
  check_contains "harness/checks/check_local_harness_setup.sh" 'check-project-shared-skills\.sh' "doctor local harness setup checks shared skills checker"
  check_contains "harness/checks/check_local_harness_setup.sh" 'install-project-skills\.sh' "doctor local harness setup checks shared installer entry"
  check_contains "harness/checks/check_local_harness_setup.sh" 'uninstall-project-skills\.sh' "doctor local harness setup checks shared uninstaller entry"
  ...
  check_contains "scripts/check-skill-sync-reminder.sh" 'check-skill-sync-reminder\.sh' "skill sync script delegates to shared harness wrapper"
  check_contains "scripts/lib/check-consistency/workflow.sh" 'check-feedback-archive-state\.sh' "workflow governance checks feedback archive state through shared harness wrapper"
  ...
  check_contains "docs/architecture/README.md" 'code/backend/internal/shared/architecture_test\.go' "architecture README references backend shared guardrail test"
